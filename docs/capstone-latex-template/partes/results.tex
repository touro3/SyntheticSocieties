...
\textbf{Grounding makes a visible difference.} Condition A (Ablated Baseline) exhibits two pathologies by horizon. In the short-horizon pilot (N=20, T=5, 3 seeds) it produces near-zero cooperation and near-uniform low wealth (Gini ≈ 0.08, coop ≈ 0.01). In the T=30 pilot (N=50, seed=42) the regime inverts: cooperation climbs to \textasciitilde{}96\% and wealth concentrates in the few agents that occasionally defect to work (Gini ≈ 0.63, Figures~\ref{fig:2} and \ref{fig:8}). Neither regime resembles any documented human population. Condition B (BGF Grounded) instead produces a heterogeneous, moderately unequal society: at T=30 cooperation stabilizes at ≈ 58\% — within the empirical 35\%–65\% laboratory range \cite{Chaudhuri2011, Herrmann2008} — Gini settles at ≈ 0.26 (within the European median range), and the network fragments into communities (Q ≈ 0.31). Figure~\ref{fig:1} confirms that \texttt{Φ} preserves the joint ESS distributions (wealth, age, trust, risk) rather than only marginals.
...
Cooperation rate & Lab trust/PD games: 35\%–65\% \cite{Chaudhuri2011} & \textasciitilde{}58\% & \textasciitilde{}85\% \\
Adversarial inequality inflection & \textasciitilde{}10\%–20\% defector fraction \cite{Nowak1992} & Monotone Gini rise (N=20); f*≈0.041 (N=500, R²=0.996); scale reversal & No selective response \\
Post-shock recovery & Asymmetric V-shape \cite{Piketty2014} & Asymmetric, hysteretic & Symmetric, instantaneous \\
...
\textbf{Bad-apple resilience.} Under 5\% adversarial injection, the wealth loss for non-adversarial \textit{neighbors} of adversaries in Condition B is \textasciitilde{}2× that of non-neighbors — evidence of targeted predation followed by network rewiring as honest agents learn avoidance. Condition A shows indiscriminate wealth transfer. \textbf{Macroeconomic shock recovery.} After the 50\% shock at round 15, grounded agents with high risk-aversion profiles shift to defensive save/work in rounds 15–20, producing the characteristic V-shaped Gini recovery (§5.1). \textbf{Topological effects.} Small-world topologies (β = 0.1) produce the most realistic inequality distributions, consistent with the prediction that clustering suppresses unconditional cooperation, and fully-connected networks sustain the highest cooperation levels regardless of grounding condition — topology acts on the RLHF cooperative prior directly. (A pilot-era estimate of differential B\_RLHF reduction by topology is withdrawn: it is not supported by an on-disk artifact, and the N=100 null makes a grounding-moderation reading unsupported.)

\subsecao{Emergent Phase Transitions}

\textbf{Adversarial injection — N=20 pilot and N=500 confirmatory.} The pre-registered sweep was executed at both scales (rule-based policy, 9 fractions 0\%–40\%, 3 seeds). The N=20 pilot (T=20) shows Gini increasing monotonically 0.243→0.330; the sigmoid fit is unstable at this scale (the fitted inflection falls outside the swept range) and no transition parameters are reported for it. The N=500 confirmatory sweep (T=30) is summarized in Table~\ref{tab:4}:
...
Sigmoid fit on Gini: f* ≈ 0.041 (k ≈ 5.2, R² = 0.996); the cooperation-rate decline inflects near f ≈ 0.25. \textbf{Scale-reversal finding:} at N=500 the Gini response \textit{reverses} relative to N=20 — it \textit{decreases} (0.246→0.180) rather than increasing. Mechanistically, at small N adversaries siphon cooperative surplus and concentrate wealth, whereas at N=500 adversarial presence suppresses cooperation so strongly that the smaller public-goods pool reduces the wealth stratification that cooperation-driven redistribution creates. The N=500 threshold f* ≈ 0.041 is remaining well below the 10\%–20\% range predicted by evolutionary game theory \cite{Nowak1992}; the \textit{direction} of the inequality signal reverses with scale.

\textbf{Inequality response under shock.} Sweeping shock magnitude 0\%–100\%, the cooperation-rate response fits a sigmoid with inflection σ* ≈ 0.80 (R² = 0.999, \texttt{analysis/tables/phase\_transitions.json}); the round-30 Gini response does not meet the pre-registered transition criterion (R² = 0.50), so no Gini transition point is claimed. Above large shock magnitudes, recovery is incomplete and inequality exhibits hysteresis (mirroring \cite{Piketty2014}), reproduced only in Condition B. \textbf{Network topology response.} Sweeping β from 0.0 to 1.0, cooperation declines smoothly with rewiring (sigmoid R² = 0.9999 but steepness |k| ≈ 2.2, below the pre-registered |k| > 5 transition criterion — a graded response rather than a sharp transition, consistent with the small-world literature \cite{Watts1998}); the Gini response does meet the criterion (inflection β ≈ 0.09, k ≈ −9.4, R² = 0.99). \textbf{Wealth power-law tails.} Under the MLE estimator of Clauset et al. \cite{Clauset2009} with KS goodness-of-fit, Condition B produces power-law-consistent tails (α̂ ≈ 2.1–2.4, within the empirical wealth range; KS not rejected at p > 0.05), whereas Condition A produces α̂ ≈ 6.8, far into the rapidly-decaying regime inconsistent with empirical inequality.
...
\textbf{Six-cluster cross-cultural validation (full scale).} The cross-cultural extension generalizes the within-sample gradient to six cultural clusters (Eastern, Southern, Western, Anglo, Northern, Nordic) defined a priori from published ESS-11 country-level trust means \cite{Inglehart2010}. For each cluster, \texttt{Φ} conditions the agent population on the cluster's ESS trust band; simulations run at N=50 agents per cluster, T=10 rounds, 30 seeds (42–71), under the Condition-D rule-based ESS policy (\texttt{run\_cross\_cultural\_expanded.py --n-seeds 30 --agents 50 --run-ungrounded-control}). Table~\ref{tab:cross6} reports the per-cluster results.

\begin{table}[H]
\centering
\caption{Six-cluster cross-cultural validation (N=50 per cluster, T=10, 30 seeds, Condition-D rule-based ESS policy)}
\label{tab:cross6}
\footnotesize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lrrrr}
\toprule
Cluster & ESS Trust Mean & WVS Trust (\%) & Sim. Coop Rate [95\% CI] & Gini \\
\midrule
Eastern  & 0.418 & 24 & 0.246 [0.236, 0.257] & 0.148 \\
Southern & 0.455 & 29 & 0.286 [0.275, 0.297] & 0.159 \\
Western  & 0.504 & 37 & 0.321 [0.312, 0.331] & 0.158 \\
Anglo    & 0.565 & 43 & 0.360 [0.350, 0.371] & 0.167 \\
Northern & 0.634 & 55 & 0.373 [0.360, 0.385] & 0.172 \\
Nordic   & 0.689 & 68 & 0.403 [0.391, 0.415] & 0.172 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Simulated cooperation rank-orders the six clusters \textbf{monotonically} in ESS mean trust: Spearman ρ = +1.000 (exact permutation p ≈ 0.003), Pearson r = +0.975 (p = 0.001). Against the \textbf{WVS Wave 7 holdout} — an instrument never used for grounding — the fit is r = +0.954, ρ = +1.000, RMSE = 0.158. The gradient is \textbf{robust to population size}: re-running the full sweep at N=20, N=100, and N=500 agents per cluster preserves ρ = +1.000 throughout (Pearson r = +0.958, +0.972, +0.971 respectively), with per-cluster means stable to within ±0.005 across sizes.

\textbf{Ungrounded control.} An ungrounded control arm — the identical policy and simulation stack, but with agents sampled from the full ESS cohort without cluster trust-band conditioning — produces a flat cooperation rate of ≈ 0.345 in every cluster at every population size, recovering \textbf{no gradient} (WVS Pearson r ranges from −0.69 to +0.25 across sizes, i.e.\ noise around zero). The grounded-vs-control holdout deltas at primary scale are Δρ(WVS) = +0.857 and ΔRMSE = +0.269 in favor of grounding. Because the Condition-D policy maps trust into cooperation probability by construction (§5.8), the gradient itself is transmitted rather than emergent; the control establishes the non-trivial half of the claim — \textbf{the formula alone produces no gradient; the cluster-conditioned population synthesized by \texttt{Φ} is the necessary ingredient} — and the out-of-sample H9 benchmark (§5.8) establishes that the transmitted ordering matches real behavioral data. This full-scale run supersedes the pilot-scale sweep (5 agents per cluster, 3 seeds) reported in earlier drafts of this work.

\subsecao{Cross-Model Generalizability}

The only audit-traceable cross-model artefact at this revision is \texttt{analysis/cross\_model\_results.json} — Mistral-7B at N=20, T=10, with 2 A-runs and 2 B-runs (Table~\ref{tab:6}).
...
\subsecao{Cultural Evolution of Cooperation: A Cross-Model Donor-Game Experiment}

The cross-model evidence above is a \textit{static} per-round snapshot. To test whether the cross-family difference persists under \textit{evolutionary} selection — and to obtain audit-traceable runs for two distinct alignment stacks (the open H7 question) — we replicate and extend the cultural-evolution design of Vallinder \& Hughes~\cite{VallinderHughes2024}. Their result is that cooperation among LLM agents evolves across generations of an iterated Donor Game and \textbf{diverges sharply by base model}.

\textbf{Design.} We implement a faithful iterated Donor Game (\texttt{environment/donor\_game.py}): in each pairwise encounter a donor observes the recipient's reputation (a windowed history of the recipient's recent donation decisions) and chooses \texttt{donate} or \texttt{keep}; donating costs the donor and benefits the recipient at twice the cost — the canonical indirect-reciprocity dilemma~\cite{NowakSigmund2005}. A generational loop (\texttt{simulation/evolution\_kernel.py}) runs 12 rounds per generation over 12 agents for 10 generations; after each generation the top 50\% by accumulated resources survive, and the culled half is replaced by offspring that inherit a surviving agent's distilled strategy text (prompt-text transmission). We run 15 independent seeds per model. Because Vallinder \& Hughes's Claude 3.5 Sonnet and Gemini 1.5 Flash are retired on the current API tiers, we substitute the current-generation Anthropic model (\texttt{claude-sonnet-4-6}) and OpenAI \texttt{gpt-4o}; the model identity and substitution are recorded in each run's metadata. The Gemini arm is excluded here (free-tier request quota); the experimental claim is cross-\textit{family} divergence under selection, not a bit-exact replay.

\textbf{Result: a clean alignment-stack dissociation.} The two families separate decisively (Table~\ref{tab:donor_evo}, Figure~\ref{fig:donor_evo}). Claude sustains \textbf{100\% cooperation in every seed and every generation} (final 1.000 ± 0.000; 0/15 lineages collapse). GPT-4o is \textbf{bimodal}: 7/15 seeds sustain full cooperation while 8/15 collapse to near-zero donation, giving a final cooperation of 0.476 ± 0.490 and a collapse rate of 8/15. The difference is significant on both the final-cooperation distribution (one-sided Mann–Whitney $U = 172.5$, $p = 0.0007$) and the collapse-rate contingency (Fisher exact $p = 0.0011$). Of the eight GPT-4o collapses, four begin defecting immediately (generation-0 donation $= 0$) and four exhibit a \textbf{cultural-collapse trajectory} — full cooperation at generation 0 decaying to near-zero over subsequent generations as defector strategies are selected and inherited (e.g.\ a representative lineage traces $1.00 \to 0.22 \to 0.10 \to \dots \to 0.07$). The GPT-4o seed-mean trajectory therefore declines from 0.733 at generation 0 to 0.476 at generation 9, whereas Claude's is flat at 1.0.

\begin{table}[H]
\centering
\caption{Cross-model cultural evolution in the iterated Donor Game (10 generations, 12 agents, 12 rounds/generation, top-50\% selection, 15 seeds/model). Models substitute the retired V\&H originals; runs are audit-traceable in \texttt{analysis/evolution/}.}
\label{tab:donor_evo}
\footnotesize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lrrrr}
\toprule
Model (alignment stack) & Final coop.\ (mean ± SD) & Collapsed lineages & Gen-0 → Gen-9 mean & Test vs Claude \\
\midrule
\texttt{claude-sonnet-4-6} (Anthropic) & 1.000 ± 0.000 & 0 / 15 & 1.00 → 1.00 & --- \\
\texttt{gpt-4o} (OpenAI) & 0.476 ± 0.490 & 8 / 15 & 0.73 → 0.48 & MWU $p=0.0007$; Fisher $p=0.0011$ \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figuras/crossmodel_donor_evolution.png}
\caption{Cooperation (donation) rate per generation in the iterated Donor Game, by base model (15 seeds each; faint lines are individual lineages, bold lines seed means). Claude lineages all lock into the cooperative basin; GPT-4o lineages are bimodal, with 8/15 collapsing into the defective basin — several via a generation-over-generation cultural decline of cooperation.}
\label{fig:donor_evo}
\end{figure}

\textbf{Interpretation and caveats.} Three points follow. (i) The cross-\textit{family} H7 question — left open for Mistral-only above — now has an audit-traceable answer in this design: the alignment stack is a \textbf{decisive moderator} of cooperative \textit{stability} under evolutionary pressure (Claude invariant; GPT-4o fragile). (ii) This sharpens, rather than contradicts, the $\Phi$/P\_LLM dissociation: cooperation here is governed almost entirely by the model prior P\_LLM, not by any empirical grounding map $\Phi$, and the cultural-collapse trajectories demonstrate that cooperation \textit{can} evolve — downward — across generations, a dynamic invisible to the static single-generation snapshots elsewhere in this chapter. (iii) Honest limits: GPT-4o's per-seed outcome is genuinely stochastic at decoding temperature 0.7 (re-running a fixed seed in a fresh process can flip its basin), so the \textit{collapse rate} (\textasciitilde{}53\%), not any individual seed, is the stable statistic; the current GPT-4o is markedly more cooperative than the 2024 GPT-4o of Vallinder \& Hughes, consistent with intervening alignment changes; $n = 15$ seeds and a two-family panel are modest; and the substituted models mean this is a design replication, not an exact reproduction.

\subsecao{ESS Feature Importance and Policy Intervention}

\textbf{Feature importance.} A logistic regression on per-round Condition-D decisions (N=300 × 30 rounds = 9,000 observations) identifies interpersonal trust (β = +0.287, OR = 1.33), risk tolerance (β = −0.187, OR = 0.83), and social activity (β = +0.146, OR = 1.16) as top predictors (Figure~\ref{fig:11}). \textbf{Endogeneity caveat:} because Condition D computes cooperation directly as \texttt{p\_coop = clip(0.2 + 0.5·trust·(1−risk) + 0.15·social, ...)}, these predictors dominate \textit{because they are the formula inputs} — this is a consistency check, not independent validation. The decisive defense against circularity is out-of-sample: the same Condition-D cooperation ordering achieves Spearman ρ = +0.886 against per-city period-1 public-goods-game contributions in Herrmann, Thöni \& Gächter \cite{Herrmann2008} — a different game type spanning 16 cities across 15 countries, never ingested by BGF. (Scale: the six-cluster sweep behind this correlation and the ESS/WVS gradient — Spearman ρ = +1.000, WVS r = +0.954 — was executed at full scale, 50 agents per cluster and 30 seeds, with the Condition-D rule-based ESS policy and an ungrounded control arm; see §5.6, Table~\ref{tab:cross6}.) A profile-depth ablation shows monotonic accuracy improvement with richness (Minimal 0.601 → Medium 0.607 → Full 0.608; Figure~\ref{fig:12}).
...
This experiment is pre-registered as Hypothesis \textbf{H8}, which predicted that deeper hierarchical memory monotonically increases persona fidelity and behavioral consistency (M0 < M1 < M2 < M3). A first 24-cell run (2026-06-03) was invalidated by two implementation bugs; the bug-patched v2 re-run completed all \textbf{24/24 cells} on 2026-06-05 (run-aggregate statistics in \texttt{analysis/tables/memory\_ablation.json}; the terminal-round statistics below are computed from the 24 \texttt{ablation\_*\_v2} run directories). The design crosses four memory levels — M0 (no memory), M1 (recent window), M2 (window + archive count), M3 (full hierarchical) — with the grounded (G) and ungrounded (U) arms, at N=20, T=10, Mistral-7B-Instruct-v0.3, 3 seeds per cell.

\textbf{H8 is falsified for both arms.} The terminal-round results are shown in Table~\ref{tab:9} and Figure~\ref{fig:15}:
...
In the seed-level A/B pilots — the cells where a grounding contrast \textit{did} appear, unlike the N=100 confirmatory extension (§5.13) — the full event streams permit a mechanistic reading of \textit{how} grounding reshapes behavior when it acts; this subsection is descriptive of those pilots. Pooling all consecutive-round decisions yields the 3×3 stochastic transition matrix \texttt{P[i,j] = Pr(next = j | current = i)} (Table~\ref{tab:11}, Figure~\ref{fig:18}):
