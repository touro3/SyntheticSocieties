...
|---|---|---|---|---|
| **P — Padded** (`decision/padded_ablation_policy.py`, infra ready) `[audit: C.5 ⏳]` | Token count, structural slots | Semantic content (filler text) | "The effect is prompt length" | Token-matched no-content padding (Phase 29.1, GPU runs pending) |
...
⚠ The H_length theory makes the strong testable prediction that P ≈ B; the BGF theory predicts P ≈ A. This single comparison (Condition P vs. B at primary scale) is the most consequential outstanding experiment in the program (Phase 29.1).
