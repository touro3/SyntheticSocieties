...
| C.5 | Negative control: Padded (Condition P) | `decision/padded_ablation_policy.py` exists | ⏳ | Policy file present; **no N=500/T=30 run yet**. Runner script `scripts/run_padded_control.py` exists (per the original plan). Pre-registered Phase 29.1. |
