...
| 2026-06-03 | **H8 — memory ablation run invalidated by implementation bugs** | The 24-cell LLM-policy memory ablation run (completed 2026-06-03 14:30) is invalid for testing H8. Two bugs caused all experimental conditions to execute identically: (A) `ablation.mode=no_rag` CLI override was silently ignored in the `policy.type=llm` code path (`scripts/run_config_simulation.py` routes to `LLMPolicy` which has no `ablation.mode` parameter); (B) `memory.level` config key was not propagated to `ablation_level` in `build_prompt()` — the default value of 5 (full M3 memory) was used for all 24 cells. Evidence: `md5sum` of `summary.json` files across M0–M3 conditions is identical per seed; `round_metrics.jsonl` Gini trajectories are bit-identical (M2-ungrounded-s42 shows a minor divergence of 0.005 at T=10, attributable to a different RNG evaluation order, not to ablation). H8 pre-registered prediction (Table 7 in §8.5) remains a hypothesis, not a measurement. A corrected re-run requires patching `scripts/run_memory_ablation_llm.sh` to use `policy.type=ablated_llm` (or pass `llm.ablation_level={0,1,2,3}` explicitly) and to implement a proper `no_rag` pathway for the ungrounded condition. This deviation is logged before any re-run is attempted. |

| 2026-06-05 | **H8 — v2 results deviate substantially from pre-registered predictions (M0–M2 partial, filed before M3)** | (A) Absolute cooperation levels are 57–77% above predictions at all memory levels (M0G predicted 0.330, actual 0.583; M1G predicted 0.362, actual 0.367; M2G predicted 0.407, actual 0.367). (B) Monotonicity direction is **reversed for the grounded arm**: M0G (0.583) > M1G (0.367) ≈ M2G (0.367) across all 3 seeds, counter to the pre-registered M0<M1<M2<M3 ordering. (C) Archive-count memory (M2) produces zero measurable change vs window-only memory (M1): M2G = M1G exactly at every seed and metric. (D) The grounded/ungrounded contrast reverses sign at M1: M0 has G>U (Δ=+0.167), M1 has G<U (M1G=0.367 < M1U=0.633). Filed before M3 results. | The absolute-level deviation reflects the RLHF overcooperation documented throughout §8.1–§8.5 (Mistral-7B-Instruct installs a strong cooperative prior that pre-registration predicted under rule-based grounding). The monotonicity reversal is a genuine empirical finding: window memory surfaces prior work/save actions, anchoring the LLM toward non-cooperative behavior. M2=M1 indicates the archive count summary adds no incremental signal detectable in a 10-round N=20 experiment. |

| 2026-06-05 | **H8 — v2 M3 results: full hypothesis falsified for both arms (final, 24/24 cells)** | **M3 complete results (all 3 seeds, N=20, T=10, Mistral-7B):** M3G: coop=0.367±0.062, B_RLHF=0.072±0.034; M3U: coop=0.450±0.000, B_RLHF=0.122±0.008. (A) M3G (0.367) < M0G (0.583) — full hierarchical memory does not rescue grounded arm monotonicity; H8 falsified. (B) Grounded arm final pattern: M0G(0.583) > M1G(0.367) = M2G(0.367) = M3G(0.367). (C) Ungrounded arm inverted-U: M0U(0.417) < M1U(0.633) = M2U(0.633) > M3U(0.450). (D) M3U three-seed convergence to exactly 0.450 — RLHF attractor stabilisation under full memory. (E) B_RLHF global minimum at M3G (0.072) — full grounded memory nearly eliminates action-distribution concentration. (F) Neither arm satisfies M0 < M1 < M2 < M3. **H8 is fully falsified.** | M3G did not exceed M0G, confirming the pre-M3 interim verdict. The inverted-U for the ungrounded arm (M3U below M1U/M2U) was unexpected: full memory suppresses the s7 RLHF mini-cascade that drove M1U/M2U to 0.633. B_RLHF minimum at M3G is mechanistically consistent with full memory providing enough behavioral self-context to counteract the RLHF cooperative prior, not by aligning with empirical distributions but by suppressing action concentration entirely. Deviation from every quantitative prediction in Table 7 is confirmed at all 8 cells. |
