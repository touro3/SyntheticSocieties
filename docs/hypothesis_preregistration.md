...
| 2026-06-05 | **H8 — v2 results deviate substantially from pre-registered predictions** | (A) Absolute cooperation levels are 57–77% above predictions at all memory levels (M0G predicted 0.330, actual 0.583; M1G predicted 0.362, actual 0.367; M2G predicted 0.407, actual 0.367). (B) Monotonicity direction is **reversed for the grounded arm**: M0G (0.583) > M1G (0.367) ≈ M2G (0.367) across all 3 seeds, counter to the pre-registered M0<M1<M2<M3 ordering. (C) Archive-count memory (M2) produces zero measurable change vs window-only memory (M1): M2G = M1G exactly at every seed and metric. (D) The grounded/ungrounded contrast reverses sign at M1: M0 has G>U (Δ=+0.167), M1 has G<U (M1G=0.367 < M1U=0.633). M3 data pending. | The absolute-level deviation reflects the RLHF overcooperation documented throughout §8.1–§8.5 (Mistral-7B-Instruct installs a strong cooperative prior that pre-registration predicted under rule-based grounding). The monotonicity reversal is a genuine empirical finding: window memory surfaces prior work/save actions, anchoring the LLM toward non-cooperative behavior. M2=M1 indicates the archive count summary adds no incremental signal detectable in a 10-round N=20 experiment. These deviations are reported honestly; H8 appears falsified for the grounded M0→M2 segment. M3 results (6 cells pending as of 2026-06-05) will determine whether the full claim is falsified or partially supported. |
