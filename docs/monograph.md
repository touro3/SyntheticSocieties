...
**N=100, 10-seed confirmatory extension (primary evidence).** The pre-registered extension (`experiments/mx_{A,B}_s{1..10}`) finds the grounded and ungrounded arms statistically indistinguishable on every primary metric: cooperation 0.461 vs 0.455 (MWU p = 0.91), Gini 0.718 vs 0.715 (p = 0.85), mean wealth 177.3 vs 174.6 (p = 0.35). Only the composite BRM shifts in the predicted direction by +0.016 (within the within-condition SD of 0.022). This is the falsification of H2 at primary scale and the empirical core of the Φ/P_LLM dissociation (Chapter 6).[^essweight]

[^essweight]: A subset of these confirmatory seeds drew the synthetic population from the ESS microdata before survey post-stratification weights (`anweight`) were applied (audit item A1.1, fixed 2026-05-25; the weighted draw is now the default code path). Because the A/B contrast applies the identical population-synthesis map `Φ` to both arms, any residual unweighting bias enters both conditions symmetrically and cancels in the contrast: the dissociation — a null *difference* between grounded and ungrounded arms — is invariant to it, though the absolute cooperation level may shift by a small amount under fully weighted sampling.
...
- **N=500 multi-seed cascade (GPU-bound).** Seeds s3–s10 of the N=500 cascade would convert the cascade from exploratory to confirmatory (≈ 20–30 h GPU per seed). A third seed per arm was attempted but both runs wedged on the CUDA generation kernel after the cooperation cascade saturated (the action-collapse failure mode documented for the N=500 regime); the two-seed-per-arm result of §5.13 therefore stands as the reported evidence, and the additional seeds remain future work.
