...
**Headline result.** The pre-registered 10-seed confirmatory extension (N=100, T=30, Mistral-7B-Instruct-v0.3, seeds 1–10; §8.1) **does not detect a Condition A vs Condition B contrast on any primary metric**: cooperation 0.455 ± 0.044 (A) vs 0.461 ± 0.042 (B) with MWU p = 0.91; Gini 0.715 vs 0.718 (p = 0.85); mean wealth 174.6 vs 177.3 (p = 0.35). Only BRM_composite moves in the predicted direction (+0.016, within the within-condition standard deviation of 0.022). The earlier pilot-scale contrast (single seed, N=50, T=30) appeared to reduce cooperation from 96.2 % (Cond. A) to 58.2 % (Cond. B) and Gini from 0.625 to 0.260; these magnitudes are not reproduced by the extension and should now be read as descriptive statistics of a single run rather than as confirmed treatment effects. The pilot's RLHF Bias Index values (B_RLHF: 0.712 → 0.420, −41 %) are independently withdrawn — both exceed the theoretical bounds of TV(π, π_uniform) ∈ [0, 2/3] (§6.1, audit rows A.13/A.14, forensic recompute in `analysis/reports/b_rlhf_pilot_forensic.json`); a patched-code single-seed re-execution of seed 42 yields B_RLHF = 0.2347 for *both* conditions (the runs produce byte-identical event streams, consistent with the §8.1 null). A complementary 3-seed short-horizon replication (N=20, T=5) showed the same direction across all three seeds at pilot scale, but neither it nor the T=30 single-seed pilot survives the 10-seed extension. Condition D (Rule-Based ESS, N=500, T=30, 3 seeds) independently attains Gini = 0.325 ± 0.001 within the Eurostat European empirical range (median G ≈ 0.31), providing a no-LLM calibration anchor that is unaffected by the LLM-arm non-replication.
...
Cross-model validation (§6.6) is **substantially weaker than previously presented**: a second-pass audit (2026-05-24) found that the only on-disk cross-model artefact is `analysis/cross_model_results.json`, which contains Mistral-7B alone (2 A-runs + 2 B-runs at N=20, T=10). The Qwen2.5-7B and GPT-4o-mini values in the historical Table 3 (−30.0 %, +40.3 %) have no on-disk source and are *withdrawn* pending re-execution of `scripts/run_cross_model_comparison.py` under the patched code. The Mistral on-disk data implies B_RLHF(A) ≈ 0.254 → B_RLHF(B) ≈ 0.039 at N=20 (≈ −85 % reduction), much larger than the −17.6 % previously claimed in the narrative but still consistent with the *direction* of H2 at small N. Combined with §8.1's null at N=100 (Δ_coop ≈ 0.006, MWU p = 0.91), the headline conclusion is that the within-Mistral grounding effect is highly N-dependent and is currently *not confirmed* at any scale beyond a single 2-run N=20 set. The original cross-model paragraph framing — "non-universal grounding response across alignment methodologies" — is currently unverifiable from on-disk artefacts; forensic record in `analysis/reports/cross_model_b_rlhf_forensic.json`. A memory-ablation experiment (M0–M3, 3 seeds each, Hypothesis H8) is pre-registered with the predicted monotonic persona-fidelity slope 0.609 → 0.742; the on-disk runs use `policy: mock` which bypasses the memory channel, so no LLM-policy measurement exists yet (audit row A.9 ❌). The full prediction table and run command are in **§8.5**; the LLM-policy re-run is the open critical-path experiment. Cross-cultural validation across six ESS clusters under the rule-based grounding proxy recovers the cooperation-vs-trust rank ordering perfectly (Spearman ρ = +1.000, exact p ≈ 0.003; Pearson r = +0.983, p = 0.0004), with independent WVS Wave 7 out-of-sample replication (r = +0.977); LLM-scale replication is launcher-ready (§8.3) but pending. We release the complete framework (1,538 automated tests across 122 files (as of 2026-05-24), one-command reproduction) to enable reproducible research in LLM-driven computational social science and alignment evaluation.

---

> **Summary of Contributions**
>
> 1. **BGF Framework** — Open-source, formally specified `BGF = (A, E, G, P, Φ, T)` with type-safe protocols (`PolicyProtocol`, PEP 544), dual RAG pipelines, hierarchical temporal memory with automatic reflection, and 1,538 automated tests (Section 3.4, as of 2026-05-24)
> 2. **RLHF Cooperative Bias Formalisation** — **The pre-registered 10-seed N=100 confirmatory extension (§8.1) detects no A vs B contrast on cooperation rate (Δ = +0.006, MWU p = 0.91), Gini (p = 0.85), or mean wealth (p = 0.35); only BRM_composite shifts by +0.016 (within seed variance).** The single-seed N=50 pilot's 96.2 % vs 58.2 % contrast and the 3-seed short-horizon pilot's 1 % vs 51 % contrast do not generalise. What survives as a contribution is the *formalism* `B_RLHF = TV(π, π_uniform)` together with the rule-based proxy result (§8.3, H9). The cross-model finding (§6.6, H7) is currently weakened: only the Mistral-7B row of Table 3 has on-disk data (and its magnitude conflicts with the historical narrative — see §6.6 provenance caveat); the Qwen and GPT-4o-mini rows are withdrawn pending re-execution. The within-Mistral grounding-suppression *magnitude* claimed by the pilot is withdrawn pending the N=500 LLM-scale run. *(The previously reported pilot B_RLHF values 0.712 → 0.420 are independently withdrawn — mathematically impossible under TV; see §6.1 audit rows A.13/A.14 and `analysis/reports/b_rlhf_pilot_forensic.json`.)* B_RLHF uses uniform as reference; the calibrated human-baseline version remains pending (§3.3 C2, §§5.1, 6.1).
...
> 4. **Memory Ablation Study (pre-registered, LLM-policy run pending — §8.5; audit A.9 ❌)** — Four-level (M0–M3) experimental design *predicting* monotonic persona-fidelity improvement from 0.609 (M0) to 0.742 (M3) under grounding. The 24 mock-policy runs currently on disk do not test the prediction (memory has no causal channel under `policy: mock`). The LLM-policy re-run via `scripts/run_memory_ablation_llm.sh` is the open critical-path experiment. Table 7 is reproduced in §8.5 as a pre-registered prediction, not a result; §6.9 has been replaced by a one-paragraph pending stub pointing to §8.5.
> 5. **Cross-Model Generalizability (currently weakened — see §6.6 provenance caveat)** — A multi-family validation across Mistral-7B, Qwen2.5-7B, and GPT-4o-mini was reported in the historical Table 3, but a second-pass audit (2026-05-24) found that only the Mistral row has any on-disk artefact; Qwen and GPT-4o-mini values are withdrawn pending re-execution. The contribution as currently defensible is the *formalism + benchmark spec* for per-model B_RLHF reporting (§10.x), not a confirmed three-family comparison. Forensic record: `analysis/reports/cross_model_b_rlhf_forensic.json`.
...
The **Behavioral Grounding Framework (BGF)** is the measurement and mitigation platform we develop to study this bias systematically. BGF grounds LLM agents in empirical microdata from the European Social Survey (ESS Round 11) via three mechanisms: (1) a population synthesis layer `Φ: D_ESS → Profile` that preserves joint distributions of trust, risk tolerance, and social engagement across 15+ attributes; (2) a dual Retrieval-Augmented Generation (RAG) pipeline that injects peer population norms and local social context at inference time; and (3) a hierarchical temporal memory system that enables agents to develop consistent behavioral patterns over time. We quantify grounding effectiveness with the **Behavioral Realism Metric** (BRM ∈ [0,1]). The cross-model narrative claim (reducing `B_RLHF` by 17.6 % for Mistral-7B and 30.0 % for Qwen2.5-7B at N=20, T=10) is **substantially weakened by a second-pass audit (2026-05-24)**: only the Mistral row has any on-disk artefact, its magnitude conflicts with the historical narrative, and the Qwen / GPT-4o-mini rows are withdrawn pending re-execution (§6.6 provenance caveat; `analysis/reports/cross_model_b_rlhf_forensic.json`). The defensible empirical claim is the *rule-based* cross-cultural result: under the deterministic ESS-rule grounding proxy, the framework recovers the cooperation-vs-trust rank ordering perfectly across six ESS national clusters (Spearman ρ = +1.000, exact p ≈ 0.003). *(A previously reported "65 % reduction" derives from a superseded exploratory pilot and has been removed; see audit row A.12.)*
...
**Empirical support (post second-pass audit).** The only on-disk cross-model artefact (Mistral-7B, N=20, 2 runs per arm; `analysis/cross_model_results.json`) shows `π_M(cooperate) ≈ 0.588 > 1/3` in Condition A, confirming the existence claim for one family with auditable data. The historical Table 3 reported the same existence claim for Qwen2.5-7B (`coop = 0.540`) and GPT-4o-mini (`coop = 0.495`), but those rows have **no on-disk artefact** and are withdrawn pending re-execution (§6.6 provenance caveat; `analysis/reports/cross_model_b_rlhf_forensic.json`). The conjecture should therefore be read as "supported by one on-disk family (Mistral), historically reported for two further families pending re-execution"; the cross-family universality claim is **not currently audit-verified**.
...
*Figure 2: Condition A (Ungrounded LLM) vs. Condition B (ESS-Grounded LLM) — primary phase_c_comparison experiment (N=50 agents, T=30 rounds, seed=42, Mistral-7B-Instruct-v0.3). Source: `analysis/paper_numbers.json`, generated by `scripts/fix_figure2_canonical.py`. (A) Key metrics: cooperation rate (A: 0.962 vs B: 0.582), Gini (A: 0.625 vs B: 0.260). **The B_RLHF values shown in the cached figure (A: 0.712, B: 0.420) are withdrawn** — both exceed the theoretical bound of TV(π, π_uniform) ∈ [0, 2/3] (§6.1, audit rows A.13/A.14; forensic recompute in `analysis/reports/b_rlhf_pilot_forensic.json`); the patched-code re-execution of seed 42 yields B_RLHF = 0.2347 for both conditions. (B) Action distribution as recorded in this single-seed run. (C) Lorenz curves derived from fitted Gini values. (D) Summary statistics table with data provenance. **Note: The A vs B contrast shown here does not survive the pre-registered 10-seed N=100 extension (§8.1, MWU p = 0.91 on cooperation); a patched-code re-execution of seed 42 produces byte-identical events.jsonl streams across A and B (forensic JSON, above). The figure is retained as historical pilot data; a multi-seed redraw with confidence bands is pending under §8.1.**
...
**What the synthesis tells us.** (1) Across the small pilot studies pooled here, the grounding effect is *directionally consistent* — every pooled point estimate sits in the predicted direction. (2) Between-study heterogeneity is high (I² ≥ 70% on two of three outcomes), reflecting the differing *N*, *T*, and model families. (3) The wide pooled CIs (covering zero for cooperation and Gini) reflect *k* = 2 — not low effect magnitude. **Interpretive caution, now strengthened by §8.1:** a random-effects meta-analysis over k=2 studies with I²≥70% provides almost no information beyond the individual results; the §8.1 10-seed N=100 extension is the larger study (n=10 per arm rather than k=2 underpowered pilots) and **does not show the cooperation- or Gini-arm contrasts pooled here**. The synthesis should therefore be read as a methodological aggregation exercise on pilot data that has since been superseded, not as primary evidence for H1/H2. It will become meaningful when k≥5 *and* each constituent study has the multi-seed structure of §8.1. B_RLHF values in this meta-analysis draw from the cross-model run (k=1 for Mistral), which remains the only internally verified pre-§8.1 value; the T=30 pilot B_RLHF is under audit review (§6.1, rows A.13/A.14), and the §8.1 extension implies that any single-seed Mistral B_RLHF point estimate — even if it were arithmetically valid — does not generalise.
...
Under a uniform prior and the *pilot* evidence alone, the posterior probability that BGF grounding moves cooperation closer to the empirically observed human range is indistinguishable from 1 to four decimals. **This posterior was conditioned on the pilot magnitudes and is invalidated by the §8.1 10-seed extension**: with the extension's Δ≈0.006 on cooperation rate at MWU p = 0.91, the same Bayesian update at n = 10 yields a posterior on "grounding shifts cooperation toward the human range" that is dominated by seed variance rather than concentrated near 1. The original calculation in this paragraph should be read as the conditional posterior under the pilot data (k=2 short-horizon studies + single-seed T=30), now superseded by the larger §8.1 sample. The N=500 LLM-scale run remains pending and is the next data point that could move the posterior in either direction.

### 6.0a Post-Audit Replication: Infrastructure Fixes and Re-Run of the T=30 Pilot

Three logging- and inference-infrastructure bugs were discovered on 2026-05-23 during the gpu3 audit re-run and are disclosed here before any new numbers are reported. All three are now patched (commits subsequent to `c73554e`); the patches do not alter scientific code paths (population synthesis, policy decision, payoff computation, metric aggregation) — they restore auditability of and remove a confound from the pre-existing pipeline.
...
**Replication design (`cmp_llm_s42_condB`, this paper).** With L-1, L-2, C-1 patched, we re-ran the canonical T=30 / N=50 / seed=42 Condition B configuration on a single 16 GB P100 (Mistral-7B-Instruct-v0.3, 4-bit quantisation, `BGF_MAX_BATCH_SIZE=4`, `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`, `temperature=0.7`, `memory_window=5`). Wall-clock: 10 107.7 s ≈ 2.81 h. The run produces 1 500 agent-decision events (50 agents × 30 rounds) with a non-empty `prompts.jsonl` for the first time in the project's history (511 KB at the time the data block below was extracted from `events.jsonl` directly, since the patched logger only takes effect on the next-launched run; the *complementary* in-flight `cmp_llm_s43_condB` is the first run whose `prompts.jsonl` carries the RAG-presence flags).

**Empirical run-average action distribution** (extracted from `experiments/cmp_llm_s42_condB/events.jsonl`):
...
The corrected RLHF Bias Index is `B_RLHF = TV(π, π_uniform) = ½ · Σ_a |π(a) − ⅓| = ½ · (|0.362−⅓| + |0.099−⅓| + |0.539−⅓|) = `**`0.2347`**, well inside the theoretical interval `[0, 2/3]` and consistent with the cooperation-rate triplet. This **resolves audit row A.14 by withdrawal-and-replacement** (the prior 0.420 value was mathematically inadmissible under Proposition 1; the recomputed value supersedes it). A bit-level diff of the seed-42 ablation_level=0 (`cmp_llm_s42_condA`) and ablation_level=5 (`cmp_llm_s42_condB`) re-executions shows their `events.jsonl` files are **byte-identical** (differing configs only in `experiment_id` and `llm.ablation_level`); B_RLHF(A) = B_RLHF(B) = 0.2347 at this seed, and the action triplet is identical across conditions. This is the seed-level analogue of the §8.1 N=100 null and is captured in `analysis/reports/b_rlhf_pilot_forensic.json`. The corresponding final-round Gini coefficient is `0.380` and final-round mean wealth `457.6` — the Gini is higher than the original pilot's `0.260` (canonical `paper_numbers.json`, now treated as a single-seed pre-patch historical record), placing this replication slightly above the European empirical reference (Eurostat median `G ≈ 0.31`).

**Substantive new observation — temporal phase shift in cooperation.** The run-average frequency of 0.539 cooperation masks a strongly non-stationary trajectory. Cooperation rises monotonically across rounds:
...
This is a **first-passage transition**, not a stationary distribution: with uniform initial wealth `w_0 = 50.0` every agent is forced to work (Bug-1-resistant ground truth that the system's payoff structure penalises early defection from work), and the cooperative attractor is reached only after capital accumulates beyond the minimum cooperate-amount of 5 wealth units. Reporting *run-averaged* π conflates the work-dominated warm-up regime with the cooperate-dominated steady state. We recommend that future Condition B summaries report **(a) steady-state π** (e.g. averaged over the final ⅓ of rounds, R 21 – R 30, here giving cooperation ≈ 0.89 and `B_RLHF_ss = 0.4`) **and (b) the run-average π** as separate observables; current §6.1 numbers correspond to neither cleanly. This observation also tightens the §6.4 phase-transition discussion: the *T*-axis transition we see here at single-seed-N=50 is distinct from the adversarial-fraction and shock-magnitude transitions reported in §6.4, and is a candidate for systematic *T*-sweep characterisation (pre-registered for the 10-seed extension; §8.1).

**Scope and interpretation.** This subsection is a **single-seed replication** intended to (i) re-establish a clean B_RLHF value resolving audit row A.14, (ii) disclose the L-1 / L-2 / C-1 infrastructure bugs that affect the verifiability of all prior LLM runs in this codebase, and (iii) surface the temporal phase-shift dynamics. It is *not* a re-do of the §6.1 result. The headline cooperation-rate claim of the paper is now revised by the 10-seed N=100 extension (§8.1), which finds no detectable A vs B contrast on cooperation rate, Gini, or mean wealth; this subsection's single-seed numbers should be read in that context. The accompanying seed-43 run (`cmp_llm_s43_condB`, identical configuration) was the first BGF Condition B run whose `prompts.jsonl` carries the `rag_context` audit field; the full 10-seed extension (`mx_B_s1..10`) inherits this audit field, so RAG-injection auditability is no longer in question for the §8.1 cells. Condition B claims in *pre-§8.1* sections still rest on the policy-construction-path argument for RAG presence (Limitation 14, §9), but the §8.1 extension itself is on-disk-verifiable.

Audit rows updated: **A.14** (resolved by recomputation), **A.13** (pending — Condition A replication required), **L-1 / L-2 / C-1** added.
...
> **Audit flag — B_RLHF(A) in T=30 pilot (audit row A.13 — resolved by withdrawal).** The earlier-reported `B_RLHF(A) = 0.712` for the T=30 pilot is mathematically impossible: for any 3-action distribution, `B_RLHF = TV(π, π_uniform) ≤ 2/3 ≈ 0.667` (Proposition 1, §3.2.1), and at cooperation ≈ 0.962 the maximum attainable TV is 0.629. The original `experiments/phase_c_comparison/` directory is not available on the current host, so a direct recompute on the original event log is not possible; **the value is withdrawn**. A post-patch single-seed re-execution at the same configuration (`experiments/cmp_llm_s42_condA/`, ablation_level=0, N=50, T=30) yields the action triplet `(work, save, cooperate) = (0.362, 0.099, 0.539)` and `B_RLHF = 0.2347`, internally consistent with the TV bound. See `analysis/reports/b_rlhf_pilot_forensic.json` for the full forensic record.
...
> **Audit flag — B_RLHF(B) in T=30 pilot (audit row A.14 — resolved by withdrawal).** The earlier-reported `B_RLHF(B) = 0.420` is also withdrawn: at cooperation = 0.582 the maximum achievable TV is `0.333` (verified analytically). The post-patch single-seed re-execution at the same configuration (`experiments/cmp_llm_s42_condB/`, ablation_level=5) yields the *identical* action triplet to Condition A — `events.jsonl` for the two runs is byte-identical up to the experiment_id field — and therefore `B_RLHF(B) = 0.2347` as well. The byte-identity is itself forensic evidence consistent with the §8.1 N=100 null result: at seed 42 on the patched code, the ablation_level=5 grounding context produces no observable per-action divergence from the ablation_level=0 baseline. See `analysis/reports/b_rlhf_pilot_forensic.json`.

**Statistical evidence at pilot scale.** The 3-seed short-horizon replication shows the grounding effect in the same direction on every seed for every primary metric (cooperation rate, Gini, B_RLHF). At pilot scale this would qualify as **consistent effect direction under a 3-seed design**: with n=3 per arm the exact Mann–Whitney U distribution admits a minimum two-sided p-value of 0.10, so formal significance at α=0.05 was deferred to the 10-seed extension. **The 10-seed extension has now been executed (§8.1) and does not reproduce the A vs B contrast on cooperation rate (Δ=+0.006, MWU p=0.91), Gini (Δ=+0.004, p=0.85), or mean wealth (p=0.35).** Only BRM_composite moves in the predicted direction (+0.016, within the within-condition SD of 0.022). Descriptive effect sizes exceeded 0.8 on every primary metric at 3 seeds (Hedges' g; Cohen's d systematically overestimates effect size in small samples — Hedges, 1981), but the §8.1 extension demonstrates that this 3-seed evidence did not generalise to a 10-seed N=100 replication on the post-patch code. The pilot numbers in this section should therefore be read as descriptive of single runs and as motivating evidence for the framework, not as confirmed treatment effects.

**Composite BRM (pilot vs extension).** Across the 3-seed short-horizon replication, `BRM_composite(A) ≈ 0.23 ± 0.04` vs `BRM_composite(B) ≈ 0.61 ± 0.07` (default weights w₁=0.30, w₂=0.25, w₃=0.25, w₄=0.20) — a ≈2.7× pilot-scale improvement driven primarily by the wealth-distribution (JSD) and cooperation-rate (coop_gap) sub-scores. **The 10-seed N=100 confirmatory extension (§8.1) yields a much smaller gap: `BRM_composite(A) = 0.832 ± 0.022` vs `BRM_composite(B) = 0.848 ± 0.017`, a +0.016 effect that remains in the predicted direction but is within the within-condition standard deviation.** Both arms score higher in absolute BRM at the extension scale than at the short-horizon pilot scale, which inflates the denominator and compresses the ratio: the extension result is "directionally H1-consistent, magnitude much smaller than pilot." *The four sub-component Δ_j values required for the Proposition 3 analytic certificate are emitted by `analysis/brm_sensitivity.py --emit-certificate` (audit row E.5); these should now be reported alongside the 10-seed extension components (`metrics.brm_components` in each `experiments/mx_{A,B}_s*/summary.json`) for the ordering claim to be fully auditable.*

![Macro Comparison](../analysis/figures/phase_c_macro_comparison.png)
*Figure 8: Macro-level dynamics over 30 rounds (pilot: `phase_c_comparison`, N=50, seed=42). Left panel (Gini): Condition A (dashed red) exhibits runaway inequality, climbing from G ≈ 0.08 at round 1 to G ≈ 0.63 at round 30 as public-goods payoffs concentrate wealth on the few agents who occasionally defect to work. Condition B (solid blue) stabilises at G ≈ 0.26 — within the European empirical range (Eurostat median G ≈ 0.31). Right panel (Cooperation Rate): Condition A oscillates between 0% and 100% cooperation per round (RLHF mode collapse), while Condition B maintains a stable cooperation band at ~55–65% with natural round-to-round variance. **Caveat:** this pilot is single-seed and the contrast shown here is not reproduced by the pre-registered 10-seed N=100 extension (§8.1: both arms converge to coop ≈ 0.46 and Gini ≈ 0.72 with no detectable A vs B separation). The mode-collapse vs stable-band visual contrast in this figure should therefore be read as a single-seed phenomenon rather than as a stable property of the grounding intervention. A multi-seed redraw with seed-bands is among the figures pending under §8.1.*
...
**Provenance caveat — read before Table 3.** A second-pass audit (2026-05-24) found that the only on-disk cross-model artefact is `analysis/cross_model_results.json`, which contains **Mistral-7B only** (2 A-runs + 2 B-runs at N=20, T=10) and no Qwen / GPT-4o-mini data. The Qwen and GPT-4o-mini rows below originate from a run set whose raw event logs and per-cell summaries are no longer present in the repository. Their values are documented in `docs/figure_status.md` and `docs/rlhf_multiagent_theory.md` but **cannot be reproduced from any artefact currently on disk**. Furthermore, the only on-disk Mistral data implies `B_RLHF(A) ≈ 0.254` and `B_RLHF(B) ≈ 0.039` (a ≈ −85 % reduction at N=20), not the −17.6 % shown in the rightmost column. Forensic record: `analysis/reports/cross_model_b_rlhf_forensic.json`. The table is retained for traceability with explicit flags rather than silently rewritten; a re-execution of `scripts/run_cross_model_comparison.py` under the patched code is the only route to a fully audit-traceable Table 3.

| Model | Cond. | Coop Rate | Gini | B_RLHF | ΔB_RLHF | Provenance |
|-------|-------|-----------|------|--------|---------|------------|
| Mistral-7B-Instruct-v0.3 | A | 0.900 † | 0.253 † | 0.567 † | — | †historical claim; on-disk JSON shows mean coop=0.588, B_RLHF=0.254 across 2 runs |
| Mistral-7B-Instruct-v0.3 | B | 0.800 † | 0.153 † | 0.467 † | **−17.6 %** † | †on-disk JSON shows mean coop=0.351, B_RLHF=0.039, ΔB_RLHF ≈ **−85 %** |
| Qwen2.5-7B-Instruct | A | 0.540 ‡ | 0.047 ‡ | 0.333 ‡ | — | ‡no event log or summary on disk; values **withdrawn pending re-execution** |
| Qwen2.5-7B-Instruct | B | 0.345 ‡ | 0.141 ‡ | 0.233 ‡ | **−30.0 %** ‡ | ‡withdrawn |
| GPT-4o-mini | A | 0.495 ‡ | 0.309 ‡ | 0.223 ‡ | — | ‡withdrawn |
| GPT-4o-mini | B | 0.590 ‡ | 0.204 ‡ | 0.313 ‡ | **+40.3 %** ‡ | ‡withdrawn |

*Table 3 (post-second-pass audit, 2026-05-24): Cross-model comparison (N=20, T=10 for all models). †=Mistral row retains the historical narrative magnitude but conflicts with the only on-disk Mistral artefact (`analysis/cross_model_results.json`); see forensic JSON. ‡=Qwen and GPT-4o-mini rows are withdrawn (no on-disk artefact of any kind, including raw event logs); the values are preserved here for cross-reference with `docs/figure_status.md` and `docs/rlhf_multiagent_theory.md` but should not be cited as a confirmed result. ΔB_RLHF = (B_RLHF(B) − B_RLHF(A)) / B_RLHF(A).*

*Auditability note.* `B_RLHF = TV(π, π_uniform) = 0.5 × Σ |π(a) − 1/3|`. **Mistral A is internally consistent under equal w/s split** at the historical narrative numbers: coop=0.900, work=save=0.050 → TV = **0.567**, matching the table. **Qwen and GPT-4o-mini are not internally consistent under equal split:** Qwen A coop=0.540 implies TV=0.207 (claimed 0.333, Δ=0.126); GPT-4o-mini A coop=0.495 implies TV=0.162 (claimed 0.223, Δ=0.061). Without the missing event logs the discrepancies cannot be resolved — the residual 0.126 and 0.061 would require unequal w/s splits whose triplets are unrecoverable. See `analysis/reports/cross_model_b_rlhf_forensic.json` for the full reconciliation table and the recommended re-execution path.*
...
**Read the rest of §6.6 with the Table-3 provenance caveat above in mind.** What the on-disk data does support is the *existence* claim: in the only fully audit-traceable cross-model artefact (the 2-run Mistral N=20 set in `analysis/cross_model_results.json`), both A and B exhibit `B_RLHF > 0` (0.254 and 0.039 respectively), so the RLHF Cooperative Bias Conjecture (§3.2.1) is supported by what data we have. The *magnitude* claim (−17.6 % reduction) is contradicted by that same artefact, which shows ≈ −85 %; and the *cross-family* claims (Qwen, GPT-4o-mini) are withdrawn pending re-execution of `scripts/run_cross_model_comparison.py` under the patched code. The two narrative paragraphs that follow are retained as the prior-pilot interpretation; they should be read as *the story the historical Table 3 told* rather than as confirmed cross-model evidence.

**(Historical narrative — see provenance caveat.)** Mistral-7B and Qwen2.5-7B were reported to confirm H2 directionally, with Qwen showing a stronger bias reduction (−30.0 %) than Mistral (−17.6 %). Under the on-disk artefact for Mistral the reduction is much larger (≈ −85 % at N=20), but neither magnitude is reconciled with the §8.1 N=100 null (Δ_coop ≈ 0.006, MWU p = 0.91). The Qwen2.5-7B narrative magnitude is currently unverifiable.

**(Historical narrative — see provenance caveat.)** GPT-4o-mini was reported to invert the sign (+40.3 %), motivating §6.6.1's "alignment methodology as a moderating variable" framing. With no on-disk artefact, the inversion is currently unverifiable; the §6.6.1 finding therefore stands or falls on a future cross-model re-execution. Until then, the "non-universal grounding response" claim should be read as *the most parsimonious explanation given the heterogeneity reported in the historical narrative* rather than as an audit-verified finding.

### 6.6.1 Alignment Methodology as a Moderating Variable (currently unverifiable)

The original framing of this subsection — "alignment methodology is a moderating variable in the grounding response" — rests on the cross-family heterogeneity reported in the historical Table 3 (Mistral −17.6 %, Qwen −30.0 %, GPT-4o-mini +40.3 %). With the on-disk artefact reduced to Mistral-only (and showing a different magnitude than the historical narrative), the three-family heterogeneity claim is **currently unverifiable from the data in this repository**. The framing should therefore be read as a *hypothesis the historical narrative would support* rather than as a confirmed empirical finding; the three bullets below are retained for traceability and should be re-evaluated once `scripts/run_cross_model_comparison.py` has been re-executed under the patched code for all three model families.

1. **(Historical narrative; unverifiable.)** Bias-direction heterogeneity: Mistral-7B (DPO) reduced B_RLHF under grounding by 17.6 %, Qwen2.5-7B (RLHF) by 30.0 %, and GPT-4o-mini (proprietary alignment stack) reversed the sign (+40.3 %). Across three alignment families the magnitude of the grounding response spanned a ~70-percentage-point range.

2. **(Historical narrative; partly contradicted by on-disk Mistral data.)** Baseline-bias heterogeneity: native B_RLHF in Condition A was stratified by alignment methodology (Mistral 0.567, Qwen 0.333, GPT-4o-mini 0.223). The on-disk Mistral data shows B_RLHF(A) ≈ 0.254, not 0.567, so the heterogeneity ordering may not survive a re-execution.

3. **Falsifiability implication, restated.** Whether the cross-model panel falsifies the strong form of the RLHF Cooperative Bias Conjecture (universal grounding-reduction across families) is the question that a clean re-execution would settle. As of the present revision, the strongest defensible empirical claim from the on-disk data is the weaker statement "Mistral-7B exhibits `B_RLHF > 0` in Condition A and the response to grounding is N-dependent (large at N=20, ~zero at N=100; see §8.1)." Whether GPT-4o-mini inverts the sign under grounding is genuinely an open empirical question that this paper cannot resolve from on-disk artefacts.

The practical implication for the alignment community is that B_RLHF should be measured *per model family* on auditable runs; the benchmark spec released alongside this paper (§10.x) is structured to accept submissions from arbitrary models and report per-model B_RLHF with on-disk event logs for downstream verification.

![Cross-Model Comparison](../analysis/figures/cross_model_bias_comparison.png)
*Figure 10: Cross-model RLHF bias comparison for Mistral-7B (N=20, T=10 cross-model setup). **Read with the §6.6 Table 3 provenance caveat in mind**: the cached image shows the historical narrative values (0.567 → 0.467, −17.6 %) but the only on-disk artefact (`analysis/cross_model_results.json`, 2 A-runs + 2 B-runs) shows mean values 0.254 → 0.039 (≈ −85 %). The 88 %-reduction-from-an-earlier-draft footnote that previously appeared here is consistent with the on-disk JSON, not with the historical Table 3, so the "earlier draft was spurious" framing is also withdrawn — the situation is the opposite of what that footnote claimed. See `analysis/reports/cross_model_b_rlhf_forensic.json`. Qwen2.5-7B and GPT-4o-mini comparisons (Table 3, ‡ rows) are withdrawn pending re-execution of `scripts/run_cross_model_comparison.py` under the patched code. Figure source: `analysis/figures/cross_model_bias_comparison.png` (cached pre-second-pass-audit render).*
...
This subsection is pre-registered as Hypothesis H8 and **does not yet have measured results**. The 24 ablation runs currently on disk (`analysis/tables/memory_ablation.json`) were executed under `policy: mock`, which produces a deterministic action distribution that bypasses the memory channel — they cannot test the memory hypothesis. The LLM-policy re-run (`scripts/run_memory_ablation_llm.sh`, ≈6–8 GPU-h) is the open critical-path experiment. The pre-registered prediction table (formerly Table 7) and Figure 15 are reproduced in **§8.5 (Pending Experiments — Memory Ablation)** alongside the run command and analysis plan, so this section no longer mixes prediction with measurement. Audit row A.9 ❌ tracks this gap.
...
Our pilot findings *suggested* that deploying off-the-shelf instruction-tuned LLMs for social simulation is methodologically fragile in characteristic ways: in the single-seed T=30 pilot, ungrounded Mistral-7B cooperates on 96.2% of rounds with Gini = 0.625; in the 3-seed short-horizon replication the same policy degenerates in the opposite direction (coop ≈ 1%). At pilot scale this bi-modal failure pattern motivated the grounding framework. **However, the §8.1 10-seed N=100 confirmatory extension does not reproduce either failure mode**: across 10 seeds the ungrounded arm sits at coop ≈ 0.455 with no observed mode collapse in either direction, and the grounded arm is statistically indistinguishable at 0.461. The story this section tells about "RLHF alignment tax pushing agents to 96% cooperation" therefore describes a single seed in a single configuration, not a stable property of Mistral-7B at this scale. The asymmetric-payoff-timing explanation for the high pilot Gini (high-trust ungrounded agents over-investing in cooperation early) is mechanistically plausible but conditional on the 96% pilot cooperation rate, which §8.1 does not reproduce. *(The T=30 pilot B_RLHF values cited in an earlier draft — B_RLHF(A)=0.712, B_RLHF(B)=0.420 — are withdrawn (mathematically impossible under TV); a patched-code re-execution of seed 42 yields B_RLHF = 0.2347 for both conditions. §6.1 audit rows A.13/A.14; `analysis/reports/b_rlhf_pilot_forensic.json`.)*

At pilot scale BGF reduces this bias under both regimes: in the T=30 pilot cooperation falls from 96.2% to 58.2% (Figure 2); in the 3-seed short-horizon replication cooperation rises from 0.01 to 0.51 with Gini from 0.08 to 0.15. **The §8.1 extension does not reproduce the cooperation- or Gini-arm contrasts at N=100, 10 seeds**: only BRM_composite shifts in the predicted direction (+0.016, within seed variance). The mechanistic claim that "ESS-calibrated trust and risk parameters impose empirically bounded priors on the action distribution" is therefore a *hypothesis the pilot data motivated* and the larger study did not confirm at headline magnitudes; the three component mechanisms ((1) persona conditioning, (2) RAG-injected population norms, (3) hierarchical temporal memory) remain implemented and individually plausible, but the §8.1 result implies their joint behavioural effect under the headline payoff structure is much smaller than the pilot suggested. Specifically: the network-modularity shift (Q from ~0.04 to ~0.31) and the macroeconomic differentiation reported elsewhere in §6 are conditioned on a 0.96 vs 0.58 cooperation contrast that this section can no longer claim as a confirmed treatment effect; they should be re-evaluated against the seed-by-seed network modularity values from the §8.1 cells before being attributed to grounding rather than to seed.
...
The hierarchical temporal memory system is designed to support behavioural consistency over the 30-round horizon. The memory ablation study is pre-registered to test this (§8.5, H8); the predicted M0 → M3 persona-fidelity slope is 0.609 → 0.742. **No LLM-policy measurement of this slope exists yet** (audit row A.9 ❌); the present text describes the architectural intent of the four memory levels, not measured outcomes. The reflection mechanism is hypothesised to be the dominant contributor — without it (M0), agents would exhibit effectively Markovian behaviour. Whether this prediction holds awaits `scripts/run_memory_ablation_llm.sh`.
...
#### 8.1.2 Pilot vs extension: what changed

We are unable, on the basis of these 20 runs alone, to attribute the gap between pilot and extension to a single cause. The relevant axes that differ between the two are:

1. **Population scale.** Pilot N=50 (single seed) and short-horizon N=20 vs. extension N=100. The §8.1 protocol block as previously written referenced "N=500" in places and `--agents 50` in the launch command — neither matches what was actually executed. The N=500 LLM-scale run remains pending; whether the pilot's effect re-emerges at N=500 is an empirical question this extension does not resolve.
...
#### 8.1.3 Status of the pilot-scale claims

The abstract, Key Claim 2, §4.1 power analysis, §6.1 (statistical-evidence + composite-BRM blocks), the Figure 2 / Figure 8 / Figure 17 captions, the §6.2 Bayesian posterior + meta-analytic synthesis paragraph, §7.1, Limitation 10, and the §10 conclusion have been edited in the **first rigor pass (2026-05-24, commit `0dc9427`)** to flag the non-replication and reframe the pilot magnitudes (96.2% → 58.2% cooperation, 0.625 → 0.260 Gini, ≈2.7× BRM ratio) as descriptive statistics of single runs rather than as confirmed treatment effects. The "10-seed extension transforms §§6.1–6.4 from pilot-level to confirmatory-level evidence" framing previously used in this section is withdrawn. The **second rigor pass (2026-05-24, this revision)** additionally edited: §6.6 Table 3 + surrounding prose + §6.6.1 + Figure 10 caption + Conjecture support paragraph + §10 conclusion opening + §10 contributions items 2 and 5 + abstract §1 narrative cross-model sentence + §8.5 Table 7 reader callout + Figure 17 caption (now points to Table 8 only) + Limitation 14 mitigation note + Limitation 15 scope extension + new Limitation 18 (cross-model provenance), and updated audit-trail counts (tests: 1,441 → 1,538; experiments: 192 → 236; LOC: 71,500 → 72,475) in the abstract, §3.12.1, §4 setup table, and §10 contributions item 1. Forensic records: `analysis/reports/b_rlhf_pilot_forensic.json` (first pass), `analysis/reports/cross_model_b_rlhf_forensic.json` (second pass).

**Per-hypothesis status update** (canonical register: Table 8 in §8.1 above): **H1** (ΔBRM_composite > 0) survives as a directional finding of small magnitude (+0.016, within seed variance). **H2 (grounding reduces `B_RLHF` within Mistral-7B) is falsified at N=100 in the §8.1 extension** — both arms produce statistically indistinguishable cooperation rates (MWU p = 0.91), so `B_RLHF(A) ≈ B_RLHF(B)` under any reference distribution. The N=500 LLM-scale run is the remaining route to either restore or definitively refute the pilot magnitude; until it lands, H2 should be regarded as falsified at the only sample size where it has been formally tested. **H5** (trust gradient under rule-based proxy), **H7** (cross-model heterogeneity), and **H9** (vs Herrmann 2008 PGG) are unaffected because they derive from separate designs; H7's within-Mistral row should now be read alongside this extension rather than alongside the single-seed pilot.
...
**Cross-hypothesis status snapshot.** A *current-state* forest plot exists (`analysis/forest_plot.py`, `analysis/tables/forest_plot.json`, audit row E.6) aggregating effect sizes for H1–H9 from the canonical sources in `docs/evidence_audit.md`. With the §8.1.1 results in hand, the status of each hypothesis at the published artefact level is: **H1 (ΔBRM_composite > 0)** — pilot value +0.235 (§3.3 C3, 3-seed short-horizon); §8.1 10-seed N=100 extension value +0.016 (within seed variance). Directionally supported at both scales; magnitude collapses by ~15× between scales. **H2 (ΔB_RLHF reduction within Mistral-7B)** — not supported by §8.1 on the headline TV-distance reading, since both arms produce ≈identical cooperation rates and therefore ≈identical B_RLHF under any reference distribution. The pilot pointwise reduction is not reproduced; pre-registered claim is *falsified* at N=100 pending the N=500 LLM-scale run. **H5 (Trust gradient under rule-based proxy)** — Spearman ρ = +0.800 group-level; ρ = +0.781 at the seed-level continuous design, p < 0.0001 (§6.5). Unaffected by §8.1 (separate experimental design). **H7 (Cross-model ΔB_RLHF)** — Mistral-7B −17.6%, Qwen2.5-7B −30.0%, GPT-4o-mini +40.3%; non-universal (§6.6.1). The Mistral-7B cross-model component is from the N=20, T=10 cross-model run, not the §8.1 N=100, T=30 matrix — these are two separate Mistral conditions and §8.1 does not directly falsify the cross-model row, but it does suggest that the −17.6% Mistral cross-model number may not generalise to the larger scale. **H9 (vs Herrmann 2008 PGG contributions)** — Spearman ρ = +0.886, exact p = 0.033 (§8.3, rule-based proxy). Unaffected by §8.1. **H3 (Gini-in-range), H4 (Δ modularity), H6 (bad-apple localisation), H8 (persona-fidelity slope)** — remain *pending* or *partial*. *(Earlier-draft cross-model citation of −84.7% does not match any value in Table 3 — audit row C.h2.)* Effect-size units are heterogeneous (Hedges' g, Spearman ρ, relative ΔB_RLHF), so the descriptive pooled value is presentational rather than a meta-analytic estimator. The forest plot at `analysis/figures/forest_plot.png` predates §8.1 and should be regenerated with §8.1 values overlaid on the pilot points.

![Hypothesis Forest Plot](../analysis/figures/forest_plot.png)
*Figure 17 (historical, pre-§8.1; retained for traceability): Cross-hypothesis effect-size summary as it stood at pilot scale (audit row E.6). **This figure predates the §8.1 10-seed N=100 confirmatory extension and the 2026-05-24 cross-model second-pass audit.** It shows pre-extension point estimates that are now superseded — H1 (+0.235 pre-§8.1) has shrunk to +0.016, H2 is falsified at N=100, and the Mistral-7B / Qwen / GPT-4o-mini cross-model row depends on values whose on-disk provenance is now in doubt (§6.6). **The canonical post-§8.1 hypothesis register is Table 8 below — defer to Table 8 for any current-status claim.** The cached PNG is preserved for historical reference rather than as evidence; an updated render will follow an end-to-end re-execution of the cross-model and N=500 LLM-scale runs.*
...
| **H2** | ΔB_RLHF < 0 within Mistral-7B (grounding reduces TV-bias) | Pilot reduction claimed at single seed (numerically withdrawn — §6.1, A.13/A.14) | **Falsified at N=100.** §8.1.1: cooperation 0.455 (A) vs 0.461 (B), MWU p=0.91 ⇒ B_RLHF(A) ≈ B_RLHF(B). N=500 is the remaining route to either restore or definitively refute the pilot magnitude. |
...
| **H4** | Δ network modularity > 0 (grounded > ungrounded community structure) | Pilot Q: 0.04 → 0.31 (single seed) | **Pending.** Not measured in §8.1; modularity not extracted from the 20 mx_*_n100 summaries. |
...
| **H8** | Persona fidelity monotonic in memory depth (M0 → M3) | No LLM-policy measurement (mock-policy runs bypass memory channel, audit row A.9 ❌). Prediction table in §8.5. | **Pending** — `scripts/run_memory_ablation_llm.sh`. |
...
*Confirmed at α=0.05 family-wise: none. Confirmed at α=0.05 per-test (no correction): H5 (post-hoc design), H9 (rule-based proxy). Directional only: H1. Falsified at N=100: H2. Pending: H3, H4, H6, H8 at LLM scale; H7 unaffected because design is separate.*
...
> **⚠ Reader callout.** Table 7 shows *hypothesised* values (pre-registered prediction for H8), **not** measurements. No measured LLM-policy memory-ablation result exists yet; the on-disk runs use `policy: mock` (audit row A.9 ❌, bypasses the memory channel). Do not cite these numbers as a result. See the §8.5 status block above.
...
We identify eighteen categories of limitations, ordered by potential impact on validity.
...
9. **(Resolved by withdrawal-and-replacement)** The earlier text-figure inconsistency ("0.71 → 0.25 / ≈60% reduction" vs. "0.712 → 0.420 / −41%") was first aligned (audit row A.12) and then the underlying pilot B_RLHF values were found to be mathematically impossible under TV (audit rows A.13/A.14). The original `experiments/phase_c_comparison/` directory is unavailable on the current host, so a direct recompute from the canonical event log is impossible; the 0.712 / 0.420 values are **withdrawn** throughout the paper. A patched-code single-seed re-execution of seed 42 (`cmp_llm_s42_condA` and `cmp_llm_s42_condB`) yields admissible values `B_RLHF = 0.2347` for *both* conditions; the two runs are byte-identical at the event-log level. Forensic record: `analysis/reports/b_rlhf_pilot_forensic.json`. Figure 2 and its caption flag the withdrawal; §6.1 audit blocks and the abstract have been updated. Figure 10 and Table 3 (Mistral-7B cross-model: 0.567 → 0.467, −17.6%) remain unaffected — those values are internally consistent as verified in §6.6.
...
14. **(New; partial mitigation in place)** *On-disk RAG-presence auditability for prior LLM runs.* Logging bugs L-1 (PromptLogger off-by-one) and L-2 (kernel batched path drops `rag_context`) — disclosed in §6.0a — together imply that **no Condition B run in this codebase prior to 2026-05-23 carries a non-empty `prompts.jsonl` with populated `rag_context` flags**. Verification that RAG was actually injected at inference time therefore rests on a *code-path argument* (`scripts/run_config_simulation.py:285-304` unconditionally constructs and passes `GraphRAG` + `SQLRAG` when `policy.type == "llm"`) rather than on a per-record audit trail. This is a weaker form of evidence than the rest of the paper claims for itself. Both bugs are now patched; the first run with the patches in effect (`cmp_llm_s43_condB`, in flight at the time of writing) and all 10 seeds of the §8.1 extension will carry the audit field. Until that extension is complete, replication of any prior Condition B figure from raw events requires also re-running the experiment under the patched code. Bug C-1 (env-var batch-size override) affected throughput and OOM-thrashing only, not policy decisions, and is also patched. Disclosure-level transparency is intended over best-light framing here: the bugs were live for the duration of the runs that produced Figures 2, 7, 8, 10 and Tables 2–4, and the patches are dated to commits *after* those figures were generated. **Mitigation verification:** the §8.1 N=100 cells (`mx_{A,B}_n100_s{1..10}`) and the in-flight N=500 extension (`mx_{A,B}_n500_s{1..10}`) all carry on-disk-auditable `rag_context` flags in `prompts.jsonl`; pre-2026-05-23 figures (2, 7, 8, 10) and Tables 2–4 rest on code-path inspection of `scripts/run_config_simulation.py:285-304` only.
...
18. **(New, 2026-05-24 second-pass audit)** *Cross-model artefact provenance (§6.6, Figure 10, Table 3).* The historical Table 3 cited Mistral-7B −17.6 %, Qwen2.5-7B −30.0 %, and GPT-4o-mini +40.3 % as a multi-family cross-model panel. A second-pass audit found that the only on-disk cross-model artefact is `analysis/cross_model_results.json` (Mistral-only, 2 A-runs + 2 B-runs at N=20, T=10). The Qwen and GPT-4o-mini values have no on-disk source of any kind (no event log, no per-cell summary). The Mistral on-disk data also does not match the historical narrative: the JSON shows B_RLHF(A) ≈ 0.254 → B_RLHF(B) ≈ 0.039, a ≈ −85 % reduction at N=20, vs the −17.6 % previously claimed. Internal consistency under equal w/s split additionally fails for Qwen and GPT-4o-mini (computed TV differs from claimed B_RLHF by 0.061–0.126), implying the historical numbers required unrecorded non-equal action triplets to be valid. Forensic record: `analysis/reports/cross_model_b_rlhf_forensic.json`. **Required action before any further cross-model claim:** re-execute `scripts/run_cross_model_comparison.py` for all three model families under the patched code (post commit c73554e) to regenerate auditable per-cell artefacts. Until then, only the rule-based cross-cultural result (§8.3) and the Condition D Gini anchor (§8.2) are defensible cross-system findings; the cross-model panel is currently a *historical* result with reduced evidential weight.

---

## 10. Conclusion

The Behavioral Grounding Framework introduces a formally specified platform and two complementary metrics — the Behavioral Realism Metric (BRM) and the RLHF Bias Index (`B_RLHF = TV(π, π_uniform)`) — for studying empirical grounding of LLM agents in social simulation. **Two empirical results are robust to the §8.1 confirmatory extension and the second-pass audit (2026-05-24):** (1) **rule-based ESS grounding recovers cross-cultural cooperation gradients perfectly** across six ESS clusters (Spearman ρ = +1.000, exact p ≈ 0.003, §8.3); (2) **rule-based ESS grounding places simulated Gini at the Eurostat European empirical range** (Condition D, G = 0.325 ± 0.001, §8.2). The headline LLM-based claim — that ESS grounding reduces Mistral-7B cooperation from ≈ 96 % to ≈ 58 % — **is not reproduced** by the pre-registered 10-seed N=100 extension (§8.1: 0.455 vs 0.461, MWU p = 0.91); only BRM_composite shifts in the predicted direction by +0.016 (within seed variance). The cross-model multi-family claim (§6.6, historical Table 3) is **substantially weakened**: only the Mistral-7B row has any on-disk artefact, and its magnitude conflicts with the historical narrative; the Qwen and GPT-4o-mini rows are withdrawn pending re-execution. The N=500 LLM-scale run is the critical-path experiment for resolving whether the pilot effect re-emerges at primary scale or whether the N=100 null is the asymptotic regime; an audit-traceable cross-model re-execution is required before §6.6's "non-universal grounding response" framing can be re-asserted as evidence rather than as a historical hypothesis.

Our pilot-level finding was that the grounding function `Φ: D_ESS → Profile`, combined with dual-RAG context injection, hierarchical temporal memory with belief expiry, and a production-hardened inference layer, reduces cooperation from 96.2% (Cond. A) to 58.2% (Cond. B) in the T=30 single-seed pilot and from 1% to 51% across the 3-seed short-horizon replication. **The pre-registered 10-seed N=100 confirmatory extension (§8.1) does not reproduce these contrasts**: across 10 seeds per arm at N=100, T=30 on Mistral-7B, both arms converge to coop ≈ 0.46 and Gini ≈ 0.72 with MWU p > 0.85 on both metrics; only BRM_composite shifts in the predicted direction by +0.016, within the within-condition standard deviation. The pilot-level cooperation/Gini effects should therefore be read as descriptive of single runs rather than as confirmed treatment effects, and the central thesis ("ESS grounding produces large, robust shifts in LLM action distributions") is **falsified at N=100 on the headline metrics and survives only as a small directional BRM effect** pending the N=500 LLM-scale run. The cross-model `B_RLHF` reductions (Mistral-7B −17.6%, Qwen2.5-7B −30.0%, GPT-4o-mini +40.3% inversion) derive from a separate N=20, T=10 cross-model run and are *technically* unaffected by §8.1, but the within-Mistral component should now be read as the smaller-scale counterpart of a Mistral N=100 contrast that does not appear. *(The previously reported T=30 pilot B_RLHF values of 0.712 → 0.420 are withdrawn; the patched-code re-execution yields 0.2347 for both conditions — §6.1, `analysis/reports/b_rlhf_pilot_forensic.json`.)* The 3-seed short-horizon pilot composite BRM (A: 0.23 ± 0.04 → B: 0.61 ± 0.07, ≈2.7×) is replaced at 10-seed N=100 scale by A: 0.832 ± 0.022 → B: 0.848 ± 0.017 (×1.02). The memory ablation study remains pre-registered (audit row A.9 ❌; pending LLM-policy re-run via `scripts/run_memory_ablation_llm.sh` — see §8.5); the cross-cultural trust gradient (§8.3) and Condition D Gini anchor (§8.2) are the two empirical results in this paper that are *not* in question after §8.1. A full-scale N=500 confirmatory extension and a length-matched padded-prompt control are now the critical-path runs for resolving whether the pilot's effect re-emerges at primary population scale or whether the N=100 null is the true asymptotic regime.

Cross-model validation as previously narrated (Mistral-7B: −17.6 %; Qwen2.5-7B: −30.0 %; GPT-4o-mini inversion) is **currently unverifiable from on-disk artefacts** (§6.6 provenance caveat); only Mistral has any on-disk data, and its magnitude (≈ −85 % at N=20 from the only artefact, vs no detectable contrast at N=100 per §8.1) is inconsistent with the historical narrative. The phase transition analysis reveals sigmoidal inequality dynamics under adversarial pressure and hysteretic inequality under economic shocks — these results derive from separate rule-based experiments and are unaffected by §8.1 and the cross-model provenance gap.

### Contributions Summary

1. **BGF Framework**: 1,538 tests (as of 2026-05-24), type-checked interfaces via `PolicyProtocol` PEP 544, Pydantic-validated configurations, CITATION.cff, one-command reproduction pipeline (`reproduce_paper.sh`).
2. **RLHF Cooperative Bias Formalisation**: Quantified via `B_RLHF = TV(π, π_uniform)`. Cross-model heterogeneity (§6.6.1: Mistral-7B −17.6 %, Qwen2.5-7B −30.0 %, GPT-4o-mini +40.3 %) confirms that the bias exists and that grounding response is alignment-methodology-dependent. **The within-Mistral grounding-suppression magnitude is *not* confirmed at the 10-seed N=100 scale** (§8.1: cooperation 0.455 vs 0.461, MWU p=0.91, ⇒ B_RLHF(A) ≈ B_RLHF(B)); the pilot's pointwise reduction is withdrawn. Under the equal work/save split assumption, `B_RLHF = |p − 1/3|`, so `p = 1/3 ± B_RLHF`. Limitation: uniform prior is a conservative reference; human baseline (§8.4) would provide a calibrated reference.
...
4. **Memory Ablation Study (pre-registered, LLM-policy run pending; §8.5)**: Four-level (M0–M3) experimental design with hierarchical temporal memory, event-type TTL, batch flush, importance scoring, and recency-weighted reflections. The on-disk runs used `policy: mock` (bypasses the memory channel; audit row A.9 ❌) and therefore do not test the prediction. The full prediction table (formerly Table 7) is reproduced in §8.5; the LLM-policy re-run via `scripts/run_memory_ablation_llm.sh` is the open critical-path experiment for H8.
5. **Cross-Model Generalizability (currently weakened — §6.6 provenance caveat)**: The historical narrative reported a multi-family characterization (Mistral-7B, Qwen2.5-7B, GPT-4o-mini), but the only on-disk artefact (Mistral-only, N=20, 2 runs each) does not match the narrative magnitudes; Qwen and GPT-4o-mini rows are withdrawn pending re-execution under the patched code. The defensible contribution is the *formal definition* of `B_RLHF` and the benchmark spec for per-model reporting, not a confirmed three-family panel. Forensic record: `analysis/reports/cross_model_b_rlhf_forensic.json`.
