...
This paper reports the framework, the metrics, the two positive rule-based findings, and the N=100 LLM-scale convergence as the current state of evidence. Exploratory evidence at N=500 (§8.1.5, 2 seeds, partial) reveals a qualitatively different regime — a self-amplifying RLHF cooperation cascade that drives both conditions to 95.6% cooperation and Gini 0.943–0.952 by round 24–27, with B_RLHF = 0.623 (3.2× the N=100 value) — confirming that the RLHF alignment tax is scale-dependent and identifying Graph-RAG social context as a cascade amplifier. A 10-seed gap-fill confirmatory sweep is currently running (restarted 2026-06-02). Preliminary R5 data from the gap-fill sweep confirm cascade onset at or before round 5 under `repetition_penalty=1.3` (cooperation 76.0–80.8%; B_RLHF 0.427–0.475), establishing that the token-level wedge fix does not suppress the positive-feedback cascade dynamics (§8.1.5, Observation 1b). Forensic history of withdrawn pilot values, the L-1/L-2/C-1 infrastructure bugs, and the superseded Qwen2.5-7B / GPT-4o-mini cross-model rows is preserved in [`docs/appendix_audit_trail.md`](appendix_audit_trail.md).
