...
**New finding (2026-06-01): population-scale cooperation cascade and B_RLHF amplification.** The pre-registered N=500 LLM-scale extension (`mx_{A,B}_n500_s1`) was interrupted at rounds 24 and 26 respectively, following a CUDA generation hang triggered by near-universal cooperation (>92%) that saturated GPU generation queues. Crucially, the partial data preserved in `events.jsonl` (rounds 17–24 for Condition A, 17–26 for Condition B) reveals a qualitatively distinct regime from the N=100 findings: at round 17, cooperation stands at **91.8%** under both conditions — compared with **47.8 ± 5.7%** (Condition A) and **48.8 ± 4.3%** (Condition B) at the same round for N=100. By the checkpoint round (24 and 26 respectively), Gini has reached **0.943** (Condition A) and **0.950** (Condition B), versus Gini ≈ 0.715–0.718 at N=100 — and versus the rule-based baseline Gini = 0.325 ± 0.0005 at N=500 (§8.2). The aggregate B_RLHF over the observable rounds is **0.608** (Condition A) and **0.613** (Condition B), approaching the theoretical maximum of 2/3 proved in §3.2.1 Proposition 1, and representing a **3.1× amplification** relative to the N=100 estimate (B_RLHF ≈ 0.195). Critically, the cascade is **symmetric across Conditions A and B** to within 0.4 percentage points at every round — confirming that the `Φ`/`P_LLM` dissociation persists at scale and that the RLHF prior, not ESS grounding, drives the cascade. The partial N=500 data therefore does not resolve the primary disambiguating question (whether grounding re-emerges at primary scale) but introduces a new and stronger one: **B_RLHF is not a fixed model property — it amplifies supercritically with population scale**, and the cooperation cascade it produces creates not utopian equality but extreme, near-maximum wealth stratification (max/mean wealth ratio ≈ 188× at round 24). Whether the dissociation closes at higher model capacity is the question remaining for future work; see §8.1.5 for full analysis.
...
We label this the **`Φ`/`P` dissociation**: the grounding map `Φ` is effective; the LLM policy `P_LLM` does not currently translate its input variation into output variation at primary scale. Three readings remain admissible from the present data — (i) the pilot's 96 → 58 % effect was a single-seed artefact; (ii) the true LLM-grounding effect is genuinely small (consistent with the +0.016 BRM trend); (iii) an unidentified pilot-only confounder was removed by the L-1/L-2/C-1 patches — and the N=500 partial data (§8.1.5) provides additional context but does not fully disambiguate — a complete multi-seed N=500 run with full event coverage remains the open critical-path experiment.
...
**A fourth phase transition — the LLM cooperation cascade — is identified by the partial N=500 data (§8.1.5).** Unlike the three stress-test phase transitions above (which are driven by exogenous parameter sweeps), the cooperation cascade is an intrinsic, endogenously generated phase transition in the LLM action distribution as a function of population size. At N=100, cooperation grows monotonically from 18.8% at round 1 to 52.8% at round 30 — a gradual, linear-like increase. At N=500, the same model and payoff structure produces cooperation ≥ 91.8% at the earliest observable round (17), reaching 95.8–96.2% by rounds 24–26. The cascade produces a Gini of 0.943–0.950 — far beyond any level observed in the three stress-test sweeps (maximum Gini ≈ 0.45 in the bad-apple sweep) and approaching the theoretical maximum of 1.0. The cascade is also absent in the rule-based N=500 baseline (Gini = 0.325 ± 0.0005), confirming it is not a payoff-structure artifact but an emergent property of the RLHF-cooperative prior interacting with social-context feedback. See §8.1.5 for full characterisation and §9 Limitation 21 for scope-of-inference caveats.

Crucially, the emergent wealth structures documented here are *not* initialized from ESS income data (see §7.0): all agents start at zero wealth and accumulate differentially through payoff dynamics. The power-law wealth distribution in Condition B (`α̂ ≈ 2.1`–`2.4`) therefore reflects a self-organizing preferential-attachment process: grounding-induced heterogeneity in cooperation propensity creates persistent wealth asymmetries within a few rounds, after which wealthier agents can afford cooperative investments that increase their network centrality, attracting further cooperation in a Matthew effect. The cooperation cascade at N=500 represents the extreme of this process: when 95%+ of agents cooperate indiscriminately, wealth concentrates in the tiny fraction of non-cooperating agents to a degree that produces a max-to-mean wealth ratio of 188×. The absence of this extreme concentration in Condition D (rule-based, N=500, Gini = 0.325) confirms that the cascade — and the inequality it generates — is a pure LLM-RLHF phenomenon.
...
**Execution plan (snapshot 2026-05-28).** Critical-path GPU work is sequenced via `scripts/_queue_pending_experiments.sh`, which blocks on the running N=500 LLM A/B sweep (§8.1) and then dispatches the remaining four GPU-bound runs sequentially on a single GPU to avoid the simultaneous-CUDA-init wedge documented in §8.1: (i) memory ablation LLM-policy (§8.5, H8), (ii) cross-cultural LLM at 10 seeds (§8.3, H9), (iii) padded-prompt control at N=100 (closes AE1, §7.9 and §8.1.4(ii)), (iv) cross-model comparison for Qwen2.5-7B and GPT-4o-mini (§6.6, H7 cross-family). Two pending items have external blockers and are not in the queue: §8.4 (human Prolific baseline — IRB approval) and the multi-country ESS pooled refit (Limitation 13 — awaits ESS R11 MD multi-country release).
...
| **H2** | ΔB_RLHF < 0 within Mistral-7B (grounding reduces TV-bias) | Pilot reduction claimed at single seed (numerically withdrawn — see Appendix A.2) | **Falsified at N=100 and unconfirmed at N=500 (partial).** §8.1.1: cooperation 0.455 (A) vs 0.461 (B), MWU p=0.91 ⇒ B_RLHF(A) ≈ B_RLHF(B) at N=100 (B_RLHF ≈ 0.195). §8.1.5 partial N=500 evidence (single seed, rounds 17–26): cooperation 95.8% (A) vs 96.2% (B), inter-condition gap ≤ 0.4 pp; B_RLHF(A) ≈ 0.608, B_RLHF(B) ≈ 0.613 — statistically indistinguishable and both approaching the 2/3 theoretical maximum. H2 falsified at both measured scales; the absolute level of B_RLHF is strongly N-dependent (3.1× amplification from N=100 to N=500). Multi-seed replication at N=500 with full event coverage required for confirmatory inference. |
...
*Confirmed at α=0.05 family-wise: none. Confirmed at α=0.05 per-test (no correction): H5 (post-hoc design), H9 (rule-based proxy). Directional only: H1, H4 (H4 newly added: ΔQ = +0.005 at N=100, p = 0.68 — directionally consistent but not significant, pilot Q-shift not reproduced). Falsified at N=100: H2. Pending: H3, H6, H8 at LLM scale; H7 unaffected because design is separate. **§8.1 N=500 LLM-scale extension status (updated 2026-06-01 — cooperation cascade discovered).** The four N=500 LLM-scale cells (`mx_A_n500_s1`, `mx_B_n500_s1`, `mx_A_n500_s6`, `mx_B_n500_s6`) were interrupted with statuses "interrupted" before completing all 30 rounds. The proximate cause was a CUDA generation hang (100% GPU utilisation, zero syscall activity, SIGTERM ignored — only SIGKILL recovers) that occurred after cooperation rates exceeded 92% per round, saturating batch generation queues. `mx_A_n500_s1` completed 24/30 rounds; `mx_B_n500_s1` completed 26/30 rounds. The s6 cells were affected by a separate `EventLogger` truncation bug (raised as `FileExistsError` on non-empty event files unless `force=True` is passed) that left minimal recoverable data. The partial on-disk artefacts — 8 rounds of `events.jsonl` for `mx_A_n500_s1` (rounds 17–24) and 10 rounds for `mx_B_n500_s1` (rounds 17–26), plus checkpoint files at rounds 24 and 26 — constitute the available N=500 LLM evidence base. A full analysis of this partial data is provided in §8.1.5 below; it reveals a qualitatively new phenomenon (cooperation cascade) that was not anticipated from the N=100 results and is described in that section. H1/H2/H3 disambiguation at primary LLM scale with full round coverage and a multi-seed estimate remains an open critical-path experiment requiring fresh GPU runs and the CUDA-hang fix (sequence launches with ≥120 s stagger between processes).*

---

### 8.1.5 Population-Scale Cooperation Cascade: Evidence from Partial N=500 Data

**Status: PARTIAL DATA — single seed (s1), rounds 17–24 (Cond A) and 17–26 (Cond B); rounds 1–16 lost to event-rotation buffer (see §9 Limitation 14). Multi-seed replication requires fresh GPU runs.**

The recoverable N=500 `events.jsonl` records, together with checkpoint files at rounds 24 (Condition A) and 26 (Condition B), reveal a behavioural regime qualitatively unlike anything observed at N=100 or N=500 under the rule-based policy. We term this the **LLM cooperation cascade** and characterise it below.

#### 8.1.5.1 The Cascade: Quantitative Summary

| Metric | Cond A N=100 (R17, n=10 seeds) | Cond B N=100 (R17, n=10 seeds) | Cond D N=100 (R17, n=10 seeds) | Cond A N=500 (R17, s1) | Cond B N=500 (R17, s1) | Cond D N=500 (R17–30, n=10) |
|---|---|---|---|---|---|---|
| Cooperation % | 47.8 ± 5.7% | 48.8 ± 4.3% | 31.0 ± 0.0% | **91.8%** | **91.8%** | 38.6% |
| Cooperation % final | 52.8 ± 5.4% (R30) | 55.3 ± 8.5% (R30) | 37.0% (R30) | 95.8% (R24) | 96.2% (R26) | 38.6% (R30) |
| Gini (final round) | 0.715 ± 0.034 | 0.718 ± 0.034 | 0.306 ± 0.006 | **0.943** (R24) | **0.950** (R26) | 0.325 ± 0.0005 |
| B_RLHF (aggregate) | 0.195 | 0.187 | 0.099 | **0.608** | **0.613** | 0.106 |
| Mean wealth (final) | 174.6 ± 9.2 | 177.3 ± 9.4 | — | 173.7 (R24) | 176.0 (R26) | — |
| Max / mean wealth ratio | — | — | — | **188×** | **192×** | — |

*N=100 values: aggregate over 10 seeds, 30 rounds; ± denotes SD across seeds. N=500 LLM values: single seed (s1), partial data. N=500 rule-based values: 10-seed complete run (§8.2). B_RLHF = TV(π, π_uniform); theoretical maximum for |A|=3 is 2/3 ≈ 0.667.*

At round 17, cooperation under both LLM conditions at N=500 stands at **91.8%** — a 1.92× ratio relative to the N=100 mean at the same round (47.8–48.8%). By the last observable round, both conditions reach 95.8–96.2% cooperation with near-identical trajectories, confirming the cascade is RLHF-driven rather than grounding-dependent (the inter-condition gap at any round is ≤ 0.4 percentage points). The rule-based Condition D at N=500 stays at 38.6% throughout — confirming that the cascade is a property of the LLM decision channel, not of the payoff structure or population size per se.

The B_RLHF amplification is dramatic: from ≈ 0.195 at N=100 to ≈ 0.608 at N=500 — a **3.1× increase**, with the N=500 estimate at 91% of the theoretical maximum of 2/3. This implies that **B_RLHF is not a fixed property of a model's alignment training**; it is an emergent quantity that amplifies with population scale through the positive-feedback dynamics described below.

#### 8.1.5.2 Mechanism: A Positive-Feedback Cooperation Attractor

The cascade mechanism is consistent with a social contagion model amplified by the LLM's RLHF cooperative prior. In each round, agents observe the actions of their k=4 small-world neighbours (via Graph RAG context and memory) and generate decisions via LLM inference. Under RLHF alignment, the base prior for cooperation is already elevated — B_RLHF ≈ 0.195 at N=100 corresponds to cooperation ≈ 47% versus the uniform expectation of 33%. Once cooperation crosses a threshold, the Graph RAG context for most agents shows predominantly cooperative neighbours, which reinforces the RLHF cooperative prior further. This creates a positive feedback loop:

```
RLHF prior (cooperation > uniform) 
  → Round 1: initial cooperation rate above equilibrium 
  → Graph-RAG context shows majority cooperation 
  → RLHF prior + cooperative context → even higher cooperation next round 
  → ... 
  → Cascade to near-universal cooperation (≈ 96%)
```

The rule-based policy breaks this loop because Condition D agents apply fixed ESS-calibrated probabilities regardless of social context, producing the stable 38.6% cooperation rate at N=500.

Two features of the cascade are theoretically important:

**1. The cascade is scale-dependent.** At N=100, the same positive-feedback mechanism produces only a gradual increase from 18.8% (round 1) to 52.8% (round 30) — a monotone but non-cascading trajectory. At N=500, the same architecture and payoff structure produces a qualitatively different regime (cooperation > 92% by round 17 at latest). The threshold separating gradual growth from cascade lies somewhere in the range N ∈ (100, 500); isolating this critical population size is a direct pre-registered follow-up experiment.

**2. The cascade produces near-maximum inequality, not equality.** A naive reading might expect high cooperation to produce egalitarian outcomes. The data sharply contradicts this. At round 24 (Condition A, N=500), Gini = 0.943 and the max-to-mean wealth ratio is 188×. The mechanism is straightforward: in BGF's payoff structure, a cooperating agent transfers wealth to a target (−amount from source, +amount × multiplier to target), so near-universal cooperation concentrates wealth in agents who receive transfers without reciprocating — specifically those who happen to choose "work" or "save" while surrounded by cooperators. The tiny non-cooperating fraction (4–5% of agents at terminal state) accumulates wealth from hundreds of cooperating neighbours. This is a Matthew-effect amplified to its logical extreme: the cooperation cascade generates the highest inequality observed in any BGF experimental condition, including the adversarial bad-apple injection (§5.2).

Compare: N=500 LLM Gini (0.943–0.950) > N=100 LLM Gini (0.715–0.718) > N=500 rule-based Gini (0.325) ≈ Eurostat European reference (≈0.31). The LLM-cooperation cascade is pathological both as a behavioural claim (no real population cooperates at 96%) and as a distributional outcome (the resulting inequality is historically unprecedented in any modern economy).

#### 8.1.5.3 The Cascade Reinforces the Φ/P_LLM Dissociation

The cascade is symmetric across Conditions A (no RAG) and B (full ESS-RAG): at every observable round, the cooperation difference between conditions is less than 0.4 percentage points. The Gini values at the checkpoint rounds are 0.943 (A) and 0.950 (B) — a gap of 0.007, well within the seed variance of either condition at N=100. This confirms that the ESS grounding context, which is the only structural difference between the two conditions, exerts no measurable moderating effect on the cascade. The RLHF prior not only dominates at N=100 (§8.1.1, MWU p = 0.91) but also dominates the cascaded regime at N=500.

#### 8.1.5.4 Interpretation and Scope

**What this evidence supports.** The partial N=500 data establishes (i) that the cooperation cascade is a real phenomenon under the Mistral-7B + small-world BGF configuration, (ii) that B_RLHF amplifies with N in this system, and (iii) that the Φ/P_LLM dissociation persists at N=500. These conclusions rest on a single seed (s1) for each LLM condition; the s6 cells were compromised by the EventLogger bug. A multi-seed replication (seeds 1–10 for both A and B at N=500, with the CUDA-stagger fix) would convert points (i)–(iii) from single-seed observations to confirmatory claims.

**What this evidence does not support.** (a) The onset round of the cascade is unknown: events for rounds 1–16 were dropped by the event-rotation buffer (§9 Limitation 14), so we cannot establish whether cooperation was already elevated at round 1 or whether it crossed a threshold mid-run. (b) Whether the cascade is transient (cooperation could return to lower levels if the run were resumed) or stable cannot be determined from the monotone upward trajectory in the available 8–10 rounds. (c) The critical population-size threshold for cascade onset is uncharacterised.

**Revised hypothesis status.** H2 (ΔB_RLHF < 0 within Mistral-7B) remains falsified at N=100. At N=500, the partial evidence shows B_RLHF(A) ≈ B_RLHF(B) ≈ 0.61 — the null A ≈ B contrast persists at even higher absolute bias levels. The cascade does not re-open H2; it contextualises it: the grounding-vs-RLHF dissociation holds across scales, while RLHF's absolute influence amplifies. H3 (Gini ∈ Eurostat empirical range) is now additionally falsified at N=500 LLM scale, where Gini = 0.943–0.950 lies far outside any documented real-world range.

**Table 8 addendum (N=500 partial evidence).** The row for H2 in Table 8 should be read with the following update: at N=500 (partial, single seed), cooperation stands at 95.8–96.2% (A: 95.8%, B: 96.2%; inter-condition gap ≤ 0.4 pp), B_RLHF(A) ≈ 0.608, B_RLHF(B) ≈ 0.613 — statistically indistinguishable and both approaching the theoretical maximum of 2/3. The H2 falsification (B_RLHF(A) ≈ B_RLHF(B)) holds at both scales tested; the absolute level of B_RLHF is strongly N-dependent.
...
21. **(New, 2026-06-01) B_RLHF is N-dependent; the cooperation cascade invalidates single-scale inference.** The formal definition `B_RLHF = TV(π, π_uniform)` is presented in §3.2.1 as a property of the LLM-plus-grounding configuration under a fixed payoff structure. The partial N=500 data in §8.1.5 shows that B_RLHF ≈ 0.608 at N=500 versus ≈ 0.195 at N=100 — a 3.1× amplification from the same model, same payoff structure, and same network topology, differing only in population size. This violates the implicit assumption that B_RLHF is a scale-invariant alignment diagnostic. The amplification mechanism is the positive-feedback cooperation cascade (§8.1.5.2): larger populations create richer neighbourhoods of cooperating agents in the Graph-RAG context, which reinforces the RLHF cooperative prior in subsequent rounds, which increases cooperation further. The cascade is therefore an emergent system-level phenomenon, not a property of any individual agent's alignment. Three consequences follow for the framework's use as an alignment diagnostic: (a) B_RLHF estimates at one population scale cannot be extrapolated to others without characterising the cascade onset; (b) the theoretical maximum B_RLHF = 2/3 appears to be a practical upper bound that the cascade approaches from below as N grows; (c) grounding effectiveness (H2) should be evaluated not just at a single N but across the cascade-onset range, since the cascade may suppress grounding effects that are detectable pre-onset but invisible post-onset. Quantifying the functional form B_RLHF(N) over the cascade transition range is a direct pre-registered follow-up experiment.

22. **(New, 2026-06-01) N=500 partial data rests on a single seed for LLM conditions.** The cooperation-cascade analysis in §8.1.5 draws on `mx_A_n500_s1` (rounds 17–24) and `mx_B_n500_s1` (rounds 17–26). The s6 cells were compromised by the EventLogger bug and contribute no interpretable data. A single-seed estimate at N=500 carries no between-seed variance estimate and no confidence interval for any reported metric (cooperation %, Gini, B_RLHF). The rule-based N=500 baseline (§8.2) is well-powered (n=10 seeds, SD = 5×10⁻⁴), making the comparison between LLM cascade estimates and the rule-based reference somewhat asymmetric in inferential strength. All cascade-section claims should therefore be read as exploratory (single-seed observations) rather than confirmatory, and the cascade findings are labelled accordingly in §8.1.5.
...
**Open questions (critical-path, updated 2026-06-01).** The N=500 LLM-scale runs (`mx_{A,B}_n500_s1`) were interrupted at rounds 24 and 26 by a CUDA hang triggered by the cooperation cascade described in §8.1.5. The partial data (rounds 17–26 for each condition, single seed) reveals a qualitatively new regime that was not observed at N=100: cooperation at 91.8–96.2%, B_RLHF approaching 0.61 (versus ≈ 0.195 at N=100), and Gini reaching 0.943–0.950 (versus ≈ 0.715 at N=100 and 0.325 for the rule-based N=500 baseline). Three new open questions emerge from this partial evidence:

(i) **Critical population-size threshold.** The cooperation cascade occurs somewhere between N=100 (no cascade) and N=500 (cascade, onset unknown). Identifying the critical N requires a sweep N ∈ {100, 150, 200, 300, 500} under matched Mistral-7B + small-world conditions.

(ii) **Onset timing.** The event-rotation bug (§9 Limitation 14) dropped rounds 1–16 from the N=500 event logs. It is unknown whether cooperation was already cascaded at round 1 (which would suggest the cascade is driven by the RLHF prior at N=500 network density without requiring memory accumulation) or whether it builds progressively over the first 10–15 rounds. Fixing the event-rotation buffer and rerunning with the CUDA-stagger fix (≥120 s between process launches) would resolve this.

(iii) **Whether grounding moderates the cascade.** At the observable rounds 17–26, Conditions A and B differ by ≤ 0.4 percentage points — the Φ/P_LLM dissociation persists at scale. A key open question is whether, at the cascade onset (rounds 1–10), grounding produces any temporary inhibitory effect before being overwhelmed by the positive feedback loop. This requires the full event log.

A length-matched padded-prompt control (§3.9) at N=100 disambiguates whether the small +0.016 BRM trend at N=100 is grounding-driven or a prompt-length artefact. Together with the cascade characterisation experiments, these runs settle whether H1/H2 survive at primary scale. This paper reports the framework, the metrics, the two positive rule-based findings, the N=100 LLM-scale convergence, and the N=500 partial cascade discovery as the current state of evidence. Forensic history of withdrawn pilot values, the L-1/L-2/C-1 infrastructure bugs, and the superseded Qwen2.5-7B / GPT-4o-mini cross-model rows is preserved in [`docs/appendix_audit_trail.md`](appendix_audit_trail.md).
...
2. **RLHF Cooperative Bias Formalisation**: The first formal operationalisation `B_RLHF = TV(π, π_uniform)` (Proposition 1, §3.2.1) with the closed bound `B_RLHF ∈ [0, 2/3]` for `|A| = 3`. The §8.1 N=100 confirmatory extension (Mistral-7B) finds `B_RLHF(A) ≈ B_RLHF(B) ≈ 0.195` (cooperation 0.455 vs 0.461, MWU p = 0.91), so the *within-Mistral* grounding-bias-reduction prediction is **not confirmed at N=100**. The §8.1.5 partial N=500 evidence finds `B_RLHF(A) ≈ 0.608`, `B_RLHF(B) ≈ 0.613` — a **3.1× amplification** and a new finding: **B_RLHF is not a scale-invariant model property but an N-dependent emergent quantity** that approaches the theoretical maximum as population scale crosses a cascade-onset threshold (§9 Limitation 21). Limitation: uniform prior is a conservative reference; human baseline (§8.4) would provide a calibrated reference; single-seed N=500 evidence requires multi-seed replication (§9 Limitation 22).
...
(i) **Cooperation cascade characterisation (highest priority, 2026-06-01)**: (a) Fresh N=500 LLM runs (seeds 1–10 for both A and B) with CUDA-stagger fix (≥120 s between process launches), event-rotation buffer extended to cover all 30 rounds, and checkpoint fidelity validation — this converts the single-seed cascade observation in §8.1.5 into a confirmatory multi-seed finding with CIs; (b) a population-size sweep N ∈ {100, 150, 200, 300, 500} under matched Mistral-7B + small-world conditions to isolate the critical cascade onset N*; (c) early-round event recovery via the extended buffer to characterise the onset timing within each run. (ii) **Human behavioral baseline** (Section 8.4): n=30–50 Prolific participants playing the BGF game, enabling computation of `B_RLHF(π, π_human)` and a properly calibrated realism comparison. (iii) **Padded prompt control at primary scale**: N=100 (pre-cascade), T=30, 3 seeds using `decision/padded_ablation_policy.py` — closes the prompt-length confound (AE1) for the N=100 BRM trend; at N=500 (post-cascade), the cascade symmetry across A and B already rules out prompt-length as the mechanism. (iv) **Cascade inhibition experiments**: testing whether (a) temperature reduction, (b) explicit instruction to diversify action choices, or (c) a cooperation-penalty payoff modification can inhibit the cascade onset — this identifies whether the cascade is a prompt-engineering artefact or an architectural property of RLHF alignment. (v) **B_RLHF(N) functional form**: systematic measurement of B_RLHF as a function of N across the cascade-onset range, to determine whether the amplification follows a phase-transition (sigmoidal) or monotone-increasing functional form, and to estimate the critical exponent. (vi) **Cluster-specific cooperation baseline models**: fitting ESS R11 volunteering models per cultural cluster to remove the Austrian-calibration confound in cross-cultural persona fidelity evaluation (Limitation 13). (vii) **Individual-level behavioral heterogeneity reporting**: distribution of per-agent fidelity scores across the full population, including quantile plots and subgroup analysis by ESS demographic strata. (viii) **Payoff sensitivity analysis**: systematically varying the cooperation payoff ratio (currently −3/+12/|C|) to characterize whether the grounding effect and the cascade are robust to changes in social dilemma tension. (ix) **Adaptive adversarial agents**: replacing the hard-constrained steal with LLM-based strategic deception. (x) **Fine-tuning comparison**: generating synthetic ESS-behavior pairs from Condition B and fine-tuning Mistral-7B to directly compare inference-time grounding against weight-based grounding. (xi) **Larger model validation**: Llama-3.1-70B or Mistral-Large to test whether grounding effects and the cascade threshold scale with model capacity. Note: Cross-cultural LLM validation (Section 8.3) and Condition D full-scale comparison (Section 8.2) are now complete.
