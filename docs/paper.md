...
**The B_RLHF(N) multi-point sequence** (combining N=20 cross-model data, N=100 10-seed confirmatory, and N=500 exploratory cascade data) is:
...
| 500 (R5, rep_pen=1.3) | 0.427 (1 seed) | 0.475 (1 seed) | +0.048 | Cascade forming; preliminary |
| 500 (terminal, no fix) | 0.623 (2 seeds) | 0.623 (2 seeds) | 0.000 | Cascade; exploratory |

This four-point sequence reveals a rich, non-monotone pattern: grounding produces a large B_RLHF reduction at N=20, *none* at N=100, and both arms cascade to high bias at N=500 regardless. Three concurrent mechanisms are consistent with this pattern: (i) at small N, agent-level grounding signals are large relative to RLHF noise, enabling ESS personas to shift action distributions; (ii) at N=100, aggregate social context (Graph-RAG) already reports majority-cooperation norms that dominate individual persona effects; (iii) at N=500, the positive-feedback loop between the RLHF prior and Graph-RAG norms dominates all per-agent grounding signals. Whether the N=20 reduction is a *feature* of the grounding mechanism or an artefact of small-population sampling noise, and whether it reappears at intermediate N (150–500) outside the cascade regime, are both open questions addressed by the B_RLHF(N) functional-form sweep listed in §10 Future Work item (iii). The grounding response within Mistral-7B is therefore best characterised as **scale-dependent and non-linear**, not simply "absent."
...
(i) **Padded-prompt control.** The length-controlled ablation (`decision/padded_ablation_policy.py`, §3.9) at N=100 is still pending and is now even more important: if the (small) BRM trend at N=100 survives prompt-length matching, the grounding effect is real but much smaller than the pilot suggested; if it does not, the BRM trend is a length artefact. (ii) **Human behavioural baseline.** §8.4 remains open and is the only route to a properly calibrated `B_RLHF(π, π_human)` against which both arms can be scored.
...
**Status: EXPLORATORY — 2 seeds (s1, s6) per condition, partial runs; gap-fill sweep (seeds 1–10, conditions A and B) restarted 2026-06-02 at 02:36, currently at R5/30 as of 14:25 2026-06-02 (mx_A_n500_s1 on GPU 1; mx_B_n500_s2 on GPU 3). PRELIMINARY FINDING: `repetition_penalty=1.3` does NOT suppress the cooperation cascade — at R5, both conditions already show cascade-level cooperation (A: 76.0%, B: 80.8%), with B_RLHF_A ≈ 0.427 and B_RLHF_B ≈ 0.475. Cascade onset is ≤ R5 (earlier than the R9 onset documented for seeds s6 without the fix), confirming the cascade is driven by the RLHF cooperative prior combined with Graph-RAG social norm amplification rather than by token-level mode collapse. The `repetition_penalty` fix prevents the generation wedge (CUDA hang) but does not interrupt the positive-feedback cascade dynamics. Full 10-seed statistics with confidence intervals await sweep completion.**
...
|--------|-------|-------|-----------|------|---------|------|
| mx_A_n500_s1 **[NEW, rep_pen=1.3]** (heartbeat) | 5 | A | **0.760** | **0.363** | **0.427** | **NEW (2026-06-02 gap-fill restart): cascade onset ≤ R5 under `repetition_penalty=1.3`** |
| mx_B_n500_s2 **[NEW, rep_pen=1.3]** (heartbeat) | 5 | B | **0.808** | **0.379** | **0.475** | **NEW (2026-06-02 gap-fill restart): cascade onset ≤ R5 under `repetition_penalty=1.3`** |
| mx_A_n500_s6 (log) | 9 | A | 0.850 | 0.619 | 0.517 | Earliest confirmed cascade onset (no rep_pen) |
...
*†B_RLHF for rows from original interrupted runs: computed as `|p_coop − 1/3|` under the equal work/save split simplification (§3.2); the full three-way TV is within ±0.01 of this. **For the two NEW R5 rows** (gap-fill restart, rep_pen=1.3): B_RLHF is the full TV = 0.5×Σ_a|π(a)−1/3| computed directly from the three-way action distribution reported in the heartbeat artefact (no equal-split approximation). ‡Checkpoint-level data; not from event logs. Partial events.jsonl coverage due to the EventLogger rotation bug (see Limitation 15 / `docs/AUDIT_DATA_METRICS_LOGGING.md`).*
...
**Observation 1b — Cascade onset ≤ R5 under `repetition_penalty=1.3` (NEW — 2026-06-02 gap-fill restart, 1 seed per condition, provisional).** The gap-fill sweep restarted at 02:36 on 2026-06-02 with `repetition_penalty=1.3` applied to all configs. At R5 (14:25 2026-06-02), the heartbeat data for the two active seeds report: mx_A_n500_s1 — cooperation=0.760, Gini=0.363, B_RLHF=0.427 (full TV: 0.5×[|0.760−0.333|+|0.072−0.333|+|0.168−0.333|]=0.427); mx_B_n500_s2 — cooperation=0.808, Gini=0.379, B_RLHF=0.475 (full TV: 0.5×[|0.808−0.333|+|0.052−0.333|+|0.140−0.333|]=0.475). Both B_RLHF values at R5 already exceed the N=100 terminal value of 0.195. The Gini at R5 (0.363–0.379) exceeds Condition D's terminal Gini (0.325), confirming the cascade is producing inequality above the rule-based ceiling even at this early stage. These are single-seed observations and should be treated as preliminary anchors, not confirmed statistics.
...
**Observation 3 — B_RLHF is N-dependent and intra-run amplifying.** The terminal B_RLHF of 0.623 at N=500 is 3.2× the N=100 value of 0.195, and within a single N=500 run B_RLHF grows monotonically from 0.517 (R9) to 0.623 (R24/R27). This dual amplification — cross-scale and within-run — distinguishes the N=500 regime from the N=100 null and suggests that cooperation cascades in this game are a **positive-feedback** process rather than a static RLHF offset.
...
**Preliminary answer (2026-06-02 gap-fill restart, R5/30 complete, 1 seed per condition, provisional):** The gap-fill sweep under `repetition_penalty = 1.3` confirms that the cooperation cascade is **not suppressed** by the token-level fix. At R5, heartbeat artefacts record mx_A_n500_s1 at cooperation = 0.760 (B_RLHF = 0.427) and mx_B_n500_s2 at cooperation = 0.808 (B_RLHF = 0.475). For reference, the original s6 runs — which lacked the repetition-penalty fix — first exceeded 85% cooperation at R9 (B_RLHF = 0.517). The R5 values from the new sweep already approach the R9 values from the old sweep, indicating the cascade trajectory is progressing at least as fast (possibly faster, though seed-to-seed variation precludes a strong claim on 1 seed per arm). The mechanistic interpretation is consistent: `repetition_penalty = 1.3` penalises repeated *output* tokens within a single generation call, which prevents the degenerate CUDA sampling loop, but it does not alter the *input* context injected into each new prompt. The Graph-RAG social context continues to report majority-cooperation norms every round, and the RLHF cooperative prior continues to respond to those norms, driving the cascade forward unimpeded. Whether the cascade ultimately reaches the 95.6% terminal value documented in the original interrupted runs remains an open empirical question whose full answer will arrive when the current 30-round sweeps complete.

##### Quantitative Scale of the Cascade

| Metric | N=100, LLM, 10 seeds | N=500, LLM, 2 seeds (partial, no rep_pen) | N=500, LLM, 1 seed per arm (R5 only, rep_pen=1.3) | N=500, Rule-based, 10 seeds | Amplification (N=500 LLM terminal / N=100 LLM) |
|--------|---------------------|-------------------------------------------|----------------------------------------------------|-----------------------------|-----------------------------------------|
| Cooperation rate (at reported round) | 0.455–0.461 (R30) | 0.956 (R24/R27) | 0.760 / 0.808 (R5) | 0.386 (R30) | **2.08×** |
| Gini coefficient | 0.715–0.718 (R30) | 0.943–0.952 (R24/R27) | 0.363 / 0.379 (R5) | 0.325 (R30) | **1.32×** |
| B_RLHF | 0.195 | 0.623 | 0.427 / 0.475 (R5) | 0.106 | **3.19×** |
| Max/mean wealth ratio | ~30× (est.) | 188–194× | — | ~4× | **~6.3×** |

*"Partial" denotes 2-seed exploratory data with CUDA wedge interruptions; "R5 only" denotes preliminary gap-fill restart data from heartbeat artefacts (1 seed per arm, provisional — see Observation 1b above); "10 seeds" denotes completed pre-registered cells. All N=500 LLM terminal figures are from checkpoint-level data; event-log aggregates are partially corrupted by the EventLogger rotation bug and excluded. R5 gap-fill data are heartbeat artefacts under `repetition_penalty=1.3`.*
...
- **B_RLHF(N) functional form**: the cross-N sequence (N=100 terminal: 0.195; N=500 R5 under rep_pen=1.3: 0.427–0.475; N=500 R9 without rep_pen: 0.517; N=500 terminal without rep_pen: 0.623) constitutes the first empirical evidence that RLHF bias is super-linear in N in this game and amplifies intra-run. The R5 gap-fill data point (0.427–0.475) adds a fifth anchor to this sequence, placing the N=500 cascade above the N=100 terminal even at round 5. A pre-registered parametric sweep across N ∈ {100, 150, 200, 300, 500} would fit the functional form and is listed in §10 Future Work item (iii).

##### Status and Next Steps

The gap-fill sweep was relaunched 2026-06-02 at 02:36 on two P100 GPUs. **Round horizon:** mx_A_n500_s1 and mx_B_n500_s2 run T=30; all remaining seeds run T=15 (see pre-registered deviation above). Seed schedule: Condition A — s1 (T=30, GPU 1, running), s2–s10 (T=15, GPU 1, queued); Condition B — s2 (T=30, GPU 3, running), s3–s10 (T=15, GPU 3, queued); mx_B_n500_s1 (T=30, separate run on GPU 0 after current sweeps complete). As of 14:25 on 2026-06-02, both active seeds are at R5 (cascade at 76–80% cooperation).

**Preliminary findings from R5 (1 seed per condition, provisional):**
- `repetition_penalty=1.3` does **not** suppress the cascade: both conditions show cascade-level cooperation at R5 (A: 76.0%; B: 80.8%), earlier than the R9 onset documented for s6.
- B_RLHF values at R5 (0.427 for A; 0.475 for B) already exceed the N=100 terminal value (0.195), confirming the scale-dependent amplification effect persists under the fix.
- The Gini at R5 (0.363–0.379) already exceeds Condition D's terminal Gini (0.325), indicating the cascade produces inequality above the rule-based ceiling within the first 5 rounds.
...
20. **(Updated 2026-06-02)** *N=500 LLM cascade findings are partially exploratory; gap-fill sweep running.* §8.1.5 reports data from 2 seeds (s1, s6) per condition for the exploratory regime (all four cells interrupted before completing 30 rounds due to a CUDA generation wedge caused by token-level mode collapse). The event-log records for rounds 1–16 of the resumed runs are additionally affected by the EventLogger rotation bug (Limitation 15 / Appendix A.1), so the cascade's full round-by-round trajectory is inferred from checkpoint states and run logs rather than from clean event aggregates. **Do not cite §8.1.5 terminal quantitative values (B_RLHF = 0.623, Gini = 0.943–0.952) as confirmed statistics without the accompanying caveat that they rest on 2-seed exploratory data.** The 10-seed gap-fill confirmatory sweep is **currently running** (restarted 02:36 2026-06-02). **Round-horizon deviation (logged):** s1/s2 run T=30; s2–s10 (A) and s3–s10 (B) run T=15 due to monograph deadline — see §8.1.5 status block. Preliminary R5 heartbeat data confirm cascade onset under `repetition_penalty=1.3`: cooperation = 0.760 (A) and 0.808 (B), B_RLHF = 0.427 / 0.475 — both already exceeding the N=100 terminal B_RLHF of 0.195 (§8.1.5 Observation 1b). Do not cite §8.1.5 terminal quantitative values (B_RLHF = 0.623, Gini = 0.943–0.952) as confirmed multi-seed statistics; they rest on 2-seed exploratory data.
