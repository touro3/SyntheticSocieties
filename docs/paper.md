...
> 2. **RLHF Cooperative Bias Formalisation** — **The pre-registered 10-seed N=100 confirmatory extension (§8.1) detects no A vs B contrast on cooperation rate (Δ = +0.006, MWU p = 0.91), Gini (p = 0.85), or mean wealth (p = 0.35); only BRM_composite shifts by +0.016 (within seed variance).** The single-seed N=50 pilot's 96.2 % vs 58.2 % contrast and the 3-seed short-horizon pilot's 1 % vs 51 % contrast do not generalise. What survives as a contribution is the *formalism* `B_RLHF = TV(π, π_uniform)` together with the cross-model finding (§6.6, H7) and the rule-based proxy result (§8.3, H9). The within-Mistral grounding-suppression *magnitude* claimed by the pilot is withdrawn pending the N=500 LLM-scale run. *(The previously reported pilot B_RLHF values 0.712 → 0.420 are independently withdrawn — mathematically impossible under TV; see §6.1 audit rows A.13/A.14 and `analysis/reports/b_rlhf_pilot_forensic.json`.)* B_RLHF uses uniform as reference; the calibrated human-baseline version remains pending (§3.3 C2, §§5.1, 6.1).
...
*Figure 17: Cross-hypothesis effect-size summary at pilot scale (audit row E.6). **This figure predates the §8.1 10-seed N=100 confirmatory extension** and shows pre-extension point estimates only. The canonical post-§8.1 register is **Table 8 below**; the rendered PNG remains in the repository for historical reference and will be regenerated once an automated overlay script is added. Pending rows (H3, H4, H6, H8) remain placeholders until their canonical key lands in `analysis/paper_numbers.json` or the corresponding GPU / behavioural-benchmark run completes.*
...
We identify seventeen categories of limitations, ordered by potential impact on validity.
