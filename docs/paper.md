...
In a pilot-scale LLM A/B contrast (single seed, N=50 agents, T=30 rounds, Mistral-7B-Instruct-v0.3), grounding appears to reduce cooperation from 96.2% (Cond. A) to 58.2% (Cond. B) and the Gini coefficient from 0.625 to 0.260. **However, the pre-registered 10-seed confirmatory extension (N=100, T=30, Mistral-7B, seeds 1–10; §8.1) does not reproduce this contrast**: both arms converge to cooperation ≈ 0.46 (A: 0.455 ± 0.044 vs B: 0.461 ± 0.042; MWU p = 0.91) and Gini ≈ 0.72 (A: 0.715 vs B: 0.718; p = 0.85), with only BRM_composite moving in the predicted direction (+0.016, within the within-condition standard deviation). The pilot numbers should now be read as descriptive statistics of a single run rather than as confirmed treatment effects. The RLHF Bias Index values previously reported for the pilot (B_RLHF: 0.712 → 0.420, −41%) are independently flagged as mathematically impossible under the correct formula TV(π, π_uniform) ∈ [0, 2/3] (§6.1, audit rows A.13/A.14). A complementary 3-seed short-horizon replication (N=20, T=5) shows the same cooperation-suppression direction across all three seeds, but neither this nor the T=30 single-seed pilot survives the 10-seed extension. Condition D (Rule-Based ESS, N=500, T=30, 3 seeds) independently attains Gini = 0.325 ± 0.001 within the Eurostat European empirical range (median G ≈ 0.31), providing a no-LLM calibration anchor that is unaffected by the LLM-arm non-replication.
...
> 2. **RLHF Cooperative Bias Formalisation** — `B_RLHF = TV(π, π_uniform)` formalises the "helpful assistant" over-cooperation bias. The T=30 single-seed pilot showed ungrounded Mistral-7B cooperating on 96.2% of rounds vs. 58.2% under grounding; the 3-seed short-horizon replication showed 1% vs 51% on all three seeds. **These pilot magnitudes did not survive the pre-registered 10-seed N=100 confirmatory extension (§8.1): both arms converged to coop ≈ 0.46 with MWU p = 0.91.** Within the extension, only BRM_composite moves in the predicted direction (+0.016, within seed variance). The RLHF bias *formalism* and the cross-model finding (§6.6, H7) remain contributions; the within-Mistral grounding-suppression magnitude is now in question and requires the N=500 LLM-scale run before it can be cited as confirmed. *(The previously reported pilot B_RLHF values 0.712 → 0.420 are also independently flagged as audit failures — mathematically impossible under TV; see §6.1 audit rows A.13/A.14.)* B_RLHF uses uniform as reference; the calibrated human-baseline version remains pending (§3.3 C2, §§5.1, 6.1).
...
*Figure 2: Condition A (Ungrounded LLM) vs. Condition B (ESS-Grounded LLM) — primary phase_c_comparison experiment (N=50 agents, T=30 rounds, seed=42, Mistral-7B-Instruct-v0.3). Source: `analysis/paper_numbers.json`, generated by `scripts/fix_figure2_canonical.py`. (A) Key metrics: cooperation rate (A: 0.962 vs B: 0.582), Gini (A: 0.625 vs B: 0.260). B_RLHF values shown in the cached figure (A: 0.712, B: 0.420) are audit failures (§6.1, rows A.13/A.14) and must be regenerated from the raw action-frequency triplet before publication. (B) Action distribution: in this single-seed run the ungrounded LLM cooperates on 96.2% of rounds (RLHF bias); ESS grounding reduces cooperation to 58.2% and diversifies the action mix (B: 58.0% cooperate, 33.7% save, 8.0% work vs A: 96.2% cooperate, 3.8% work). (C) Lorenz curves derived from fitted Gini values, illustrating the shift from near-perfect equality in Condition A toward empirically plausible inequality in Condition B. (D) Summary statistics table with data provenance. **Important context:** the A vs B contrast shown here is from a single seed and **does not survive the pre-registered 10-seed N=100 confirmatory extension** (§8.1: A=0.455, B=0.461 on cooperation, MWU p = 0.91). This figure should therefore be read as a descriptive comparison of two specific runs, not as confirmed treatment effects. A multi-seed redraw with confidence bands is among the figures pending under §8.1.*
...
The corrected RLHF Bias Index is `B_RLHF = TV(π, π_uniform) = ½ · Σ_a |π(a) − ⅓| = ½ · (|0.362−⅓| + |0.099−⅓| + |0.539−⅓|) = `**`0.2347`**, well inside the theoretical interval `[0, 2/3]` and consistent with the cooperation-rate triplet. This **resolves audit row A.14** (the prior 0.420 value was mathematically inadmissible under Proposition 1; the recomputed value supersedes it). The corresponding final-round Gini coefficient is `0.380` and the final-round mean wealth is `457.6` — the Gini is higher than the previously reported pilot value of `0.260` (canonical `paper_numbers.json`), placing this replication slightly above the European empirical reference (Eurostat median `G ≈ 0.31`) rather than within it. The discrepancy is documented but not yet explained: candidate sources include (i) a different stochastic trajectory under the new batch schedule, (ii) the previously unaudited RAG-injection path, or (iii) a configuration drift in the canonical pilot. A bit-level diff of the two run configs and event streams is planned but not yet executed.
...
*Figure 17: Cross-hypothesis effect-size summary at pilot scale (audit row E.6). **This figure predates the §8.1 10-seed N=100 confirmatory extension** and shows the pre-extension point estimates: H1 +0.235, H2 −17.6% (Mistral cross-model), H5 +0.800, H7 (mixed), H9 +0.886. The §8.1 extension shrinks H1 to +0.016 and does not support H2 within the Mistral N=100 matrix; H5/H7/H9 are unaffected because they derive from separate designs. A regenerated forest plot with §8.1 values overlaid on the pilot points is among the figures pending. Pending rows (H3, H4, H6, H8) remain placeholders until their canonical key lands in `analysis/paper_numbers.json` or the corresponding GPU/behavioural-benchmark run completes.*
