...
> 6. **Memory Ablation Study (§8.5)** — Four-level (M0–M3) experimental design with hierarchical temporal memory. The 2026-06-03 24-cell run was invalidated by two implementation bugs (§8.5.1); the bug-patched v2 re-run (`tmux: h8_memory_ablation`) has now completed 19/24 cells as of 2026-06-05. **M0–M2 grounded fully characterised:** M0G (coop=0.583±0.085), M1G (coop=0.367±0.058), M2G (coop=0.367±0.058) — mean M2G = mean M1G exactly; M0G > M1G ≈ M2G across all 3 seeds, consistently violating H8 monotonicity (M0<M1<M2<M3). M0 grounded vs ungrounded Δ_coop=+0.167 (H8 direction); M1 direction inverts: M1G (0.367) < M1U (0.633, s7 outlier=0.950). M3 pending (6 cells). Remaining 8 cells.
...
| **H8** | Persona fidelity monotonic in memory depth (M0 → M3) | 2026-06-03 LLM-policy run completed 24 cells but **invalidated** by two implementation bugs (§8.5.1). Both bugs patched. V2 re-run active 2026-06-04 22:45 CEST (`tmux: h8_memory_ablation`). **19/24 cells complete as of 2026-06-05:** M0G (coop=0.583±0.085, B_RLHF=0.256±0.096), M0U (coop=0.417±0.026, B_RLHF=0.150±0.077), M1G (coop=0.367±0.058, B_RLHF=0.128±0.058), M1U (coop=0.633±0.289, B_RLHF=0.306±0.289; s7 outlier=0.950), M2G (coop=0.367±0.058, B_RLHF=0.144±0.067); M2U 1/3 done, M3 pending. | **Partial (19/24 cells, M3 pending)** — M0G > M0U by Δ=+0.167 (H8 direction); **M0G (0.583) > M1G (0.367) ≈ M2G (0.367) — grounded counter-monotonicity at all 3 seeds, 2 consecutive levels**; M2G = M1G exactly (archive count adds nothing). M1 direction inverts vs M0: M1G < M1U (0.367 vs 0.633). H8 monotonicity for grounded arm appears **falsified for M0→M1→M2 segment** (M3 may rescue if M3G > M0G). Analyze with `python scripts/analyze_memory_ablation.py --suffix _v2`. |
...
*Confirmed at α=0.05 family-wise: none. Confirmed at α=0.05 per-test (no correction): H5 (post-hoc design), H9 (rule-based proxy). Directional only: H1, H4. Falsified at N=100: H2. Pending: H3, H6 at LLM scale; H7 unaffected because design is separate. **H8 partial results (v2 re-run 19/24 cells complete as of 2026-06-05, §8.5):** M0G (coop=0.583±0.085) > M0U (coop=0.417±0.026), Δ=+0.167 (H8 direction, consistent); M1G (coop=0.367±0.058) < M0G at every seed (Δ=−0.217 mean), M2G (coop=0.367±0.058) = M1G exactly — **grounded arm violates H8 monotonicity at M0→M1→M2**; M3G pending (6 cells). M1 direction inverts: M1G (0.367) < M1U (0.633). H8 appears falsified for the M0→M2 grounded segment unless M3G substantially exceeds M0G (>0.583). **Exploratory N=500 (§8.1.5, both conditions confirmed through R29 as of 2026-06-05, R30 OOM-stalled):** H3 additionally falsified at N=500 LLM scale (Gini 0.963–0.968 at R29); H2 null extends to N=500 (both arms cascade) — gap-fill confirmed through R29: condA coop=93.8%, B_RLHF=0.605, agg_B_RLHF=0.513; condB coop=95.6%, B_RLHF=0.623, agg_B_RLHF=0.543; exploratory finding: condB consistently leads condA by 1.8–4.8 pp throughout R1–R29, directionally opposite to H2 (single seed per condition).*
...
**Status: 19/24 CELLS COMPLETE (2026-06-05).** V2 re-run (`tmux: h8_memory_ablation`, both bugs patched) progress: M0G ✓ (3/3), M0U ✓ (3/3), M1G ✓ (3/3), M1U ✓ (3/3), M2G ✓ (3/3), M2U 2/3 done (s42=0.400, s123=0.550; s7 running), M3G pending (3 cells), M3U pending (3 cells). Experiment IDs: `ablation_M{0-3}_{grounded,ungrounded}_s{42,123,7}_v2`. Analysis command (when M3 complete): `python scripts/analyze_memory_ablation.py --suffix _v2`.
...
**M2 Window+Archive Ungrounded (2/3 seeds complete, s7 running):**

| Seed | T | Coop (terminal) | Save | Work | Gini | B_RLHF_term | M1U coop | Δ (M2U−M1U) |
|------|---|-----------------|------|------|------|-------------|----------|-------------|
| 42 | 10/10 ✓ | 0.400 | 0.300 | 0.300 | 0.287 | 0.067 | 0.400 | 0.000 |
| 123 | 10/10 ✓ | 0.550 | 0.100 | 0.350 | 0.226 | 0.233 | 0.550 | 0.000 |
| 7 | running | — | — | — | — | — | 0.950 | — |

M2U s42 = M1U s42 exactly; M2U s123 = M1U s123 exactly — same pattern as grounded: archive count adds nothing at the ungrounded arm either. M3 data (both conditions) are required to assess whether H8 can be rescued at the M3 step.
