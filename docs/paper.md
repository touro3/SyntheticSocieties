...
In a pilot-scale LLM A/B contrast (single seed, N=50 agents, T=30 rounds, Mistral-7B-Instruct-v0.3), grounding reduces cooperation from 96.2% (Cond. A) to 58.2% (Cond. B), reduces the RLHF Bias Index (`B_RLHF = TV(π, π_uniform)`) from 0.712 to 0.420 (a 41% relative reduction; canonical Figure 2 / `analysis/paper_numbers.json`), and reduces the Gini coefficient from 0.625 to 0.260. A complementary 3-seed short-horizon replication (N=20, T=5) shows the same cooperation-suppression direction across all seeds (A: 0.01, B: 0.51); n=3 per arm does not admit formal two-sided significance testing via Mann–Whitney U (minimum achievable p = 0.10), so all pilot-scale results should be read as directional evidence pending the pre-registered 10-seed confirmatory extension. Condition D (Rule-Based ESS, N=500, T=30, 3 seeds) independently attains Gini = 0.325 ± 0.001 within the Eurostat European empirical range (median G ≈ 0.31), providing a no-LLM calibration anchor.
...
>
> 1. **BGF Framework** — Open-source, formally specified `BGF = (A, E, G, P, Φ, T)` with type-safe protocols (`PolicyProtocol`, PEP 544), dual RAG pipelines, hierarchical temporal memory with automatic reflection, and 1,372 automated tests (Section 3.4)
> 2. **RLHF Cooperative Bias Discovery** — `B_RLHF = TV(π, π_uniform)` formalises the "helpful assistant" over-cooperation bias; the T=30 single-seed pilot shows ungrounded Mistral-7B cooperating on 96.2% of rounds vs. 58.2% under grounding (B_RLHF 0.712 → 0.420, −41% relative; canonical Figure 2); the 3-seed short-horizon replication shows 1% vs 51% on all three seeds. Note: B_RLHF uses uniform as reference; the calibrated human-baseline version is pending (Section 3.3 C2, Sections 5.1, 6.1)
...
We explicitly address three construct validity challenges that bound the interpretation of all results.
...
**C3 — BRM weight sensitivity.** The composite BRM uses weights `w₁ = 0.30` (wealth JSD), `w₂ = 0.25` (Gini gap), `w₃ = 0.25` (cooperation accuracy), `w₄ = 0.20` (temporal stability). These weights are set by expert judgment rather than empirical calibration. To assess sensitivity, we verify that the direction `BRM(B) > BRM(A)` is preserved under two alternative weight schemes: (i) equal weights (`w = 0.25` each) and (ii) cooperation-dominant weights (`w₃ = 0.50`, others = 0.167`). Under both schemes, `BRM(B)` exceeds `BRM(A)` on every pilot seed by a margin that exceeds the within-condition seed variance, confirming that the rank ordering is weight-robust. Absolute BRM values vary by ±0.04 under these perturbations; relative magnitudes are stable. The full-simplex Dirichlet sweep (`analysis/brm_sensitivity.py`, 500 weight samples, seed 42) gives a stronger statement: **`BRM(B) > BRM(A)` holds in 100.0% of simplex samples** (mean Δ = 0.238, SD = 0.049, 5–95% quantile range [0.165, 0.321]; equal-weight reference Δ = 0.235), which exceeds the pre-registered 90%-of-simplex pass threshold (`docs/evaluation_protocol.md` §8) by a wide margin. The rank ordering `BRM(B) > BRM(A)` is therefore weight-robust across the entire convex hull of admissible weight vectors, not just the two named alternatives. Full sweep results: `analysis/tables/brm_sensitivity.json`; visualisation: `analysis/figures/brm_weight_sensitivity.png` (audit row E.5).

**C4 — Payoff design dependence.** The specific payoff structure (+8 work, +4 save, −3 cooperate with +12/|cooperators| to public pool) determines which action is individually rational and collectively optimal, and therefore directly governs what cooperation rate the game equilibrium selects. Under this parameterization, cooperative equilibria exist but are costly to individual agents — a deliberate design choice to prevent trivial universally-cooperative equilibria. However, results in Sections 6.1–6.4 may not generalize to payoff structures with different social dilemma tensions (e.g., prisoner's dilemma, stag hunt, assurance game). Readers should interpret findings as conditional on this specific game-theoretic environment. The sensitivity of the grounding effect to payoff parameterization is a priority for future work.
...
*Figure 2: Condition A (Ungrounded LLM) vs. Condition B (ESS-Grounded LLM) — primary phase_c_comparison experiment (N=50 agents, T=30 rounds, seed=42, Mistral-7B-Instruct-v0.3). Source: `analysis/paper_numbers.json`, generated by `scripts/fix_figure2_canonical.py`. (A) Key metrics: cooperation rate (A: 0.962 vs B: 0.582), Gini (A: 0.625 vs B: 0.260), and B_RLHF (A: 0.712 vs B: 0.420). (B) Action distribution: the ungrounded LLM cooperates on 96.2% of rounds (RLHF bias); ESS grounding reduces cooperation to 58.2% and diversifies the action mix (B: 58.0% cooperate, 33.7% save, 8.0% work vs A: 96.2% cooperate, 3.8% work). (C) Lorenz curves derived from fitted Gini values, illustrating the shift from near-perfect equality in Condition A (Gini=0.625 is misleadingly low due to mode-collapsed wealth) toward empirically plausible inequality in Condition B. (D) Summary statistics table with data provenance. The B_RLHF index is reduced by 41% from grounding.*
...
**Cross-hypothesis status snapshot (available now).** Although the 10-seed CI extension is the pre-registered confirmatory study, a *current-state* forest plot is already available (`analysis/forest_plot.py`, `analysis/tables/forest_plot.json`, audit row E.6), aggregating the present effect sizes for H1–H9 from the canonical sources documented in `docs/evidence_audit.md`. Hypotheses with verified effect sizes at pilot scale are H1 (ΔBRM_composite = +0.235, full-simplex robust; §3.3 C3), H2 (relative ΔB_RLHF = −84.7% for Mistral-7B; §6.6), H5 (Spearman ρ = +0.800 group-level; ρ = +0.781 at the seed-level continuous design, p < 0.0001; §6.5), H7 (mean relative ΔB_RLHF = −84.7% pooled across the cross-model panel; §6.6), and **H9 (Spearman ρ = +0.886 vs Herrmann 2008 PGG contributions, exact p = 0.033; §8.3)**. H3 (Gini-in-range), H4 (Δ modularity), H6 (bad-apple localisation), and H8 (persona-fidelity slope) remain *pending* or *partial* against their respective canonical artefacts. Effect-size units are heterogeneous (Hedges' g, Spearman ρ, relative ΔB_RLHF), so the descriptive pooled value is presentational rather than a meta-analytic estimator. CIs will be populated by the 10-seed extension below; the plot itself is `analysis/figures/forest_plot.png`.
...
9. **(Resolved)** Figure 2 and §6.1 now cite identical canonical values from `analysis/paper_numbers.json` / `phase_c_comparison` (A: coop=0.962, Gini=0.625, B_RLHF=0.712; B: coop=0.582, Gini=0.260, B_RLHF=0.420; relative ΔB_RLHF = −41%). The earlier inconsistency — where the Abstract/§6.1 quoted "0.71 → 0.25 (≈60% reduction)" against Figure 2's canonical "0.712 → 0.420" — is documented in audit row A.12 and has been aligned throughout the text. Figure 10 reports Mistral-7B cross-model values (0.567 → 0.467, −17.6%) consistent with Table 3 — distinct experiment (cross-model N=20, T=10), not a contradiction.
