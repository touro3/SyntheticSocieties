...
*Confirmed at α=0.05 family-wise: none. Confirmed at α=0.05 per-test (no correction): H5 (post-hoc design), H9 (rule-based proxy). Directional only: H1, H4. Falsified at N=100: H2. Pending: H3, H6 at LLM scale; H7 unaffected because design is separate. **H8 COMPLETE — v2 re-run 24/24 cells done (2026-06-05, §8.5): FALSIFIED for both arms.** Grounded arm: M0G(0.583) > M1G(0.367) = M2G(0.367) = M3G(0.367) — monotone decrease at M0→M1, flat M1=M2=M3; H8 monotonicity violated. Ungrounded arm: M0U(0.417) < M1U(0.633) = M2U(0.633) > M3U(0.450) — inverted-U; non-monotonic. M3G (0.367) does not exceed M0G (0.583); H8 is not rescued by full memory. B_RLHF global minimum at M3G (0.072±0.034). M3U convergence to 0.450±0.000 across all 3 seeds. **Exploratory N=500 (§8.1.5, both conditions T=30 COMPLETE 2026-06-05):** H3 additionally falsified at N=500 LLM scale (Gini 0.965–0.970 at R30 terminal); H2 null extends to N=500 (both arms cascade) — gap-fill T=30 complete: condA coop=94.0%, B_RLHF=0.607, agg_B_RLHF=0.516; condB coop=96.0%, B_RLHF=0.627, agg_B_RLHF=0.545; exploratory finding: condB consistently leads condA by 2.0–4.8 pp throughout R1–R30, directionally opposite to H2 (single seed per condition).*

---

#### 8.1.5 Population-Scale LLM Cooperation Cascade at N=500 (Exploratory, Partial Data)

**Status: EXPLORATORY — gap-fill sweep CONFIRMED THROUGH R30 FOR BOTH CONDITIONS (T=30 COMPLETE, 2026-06-05). R30 completed via OOM retry at chunk_size=1 (CUDA OOM at chunk_size=2 resolved by halving to chunk_size=1; R30 finished condA at 02:30:20, condB at 03:25:05, 2026-06-05 CEST). R30 is the confirmed terminal for both conditions (15,000 agent-action records each = 500 agents × 30 rounds).

CONFIRMED THROUGH R30 (condA, ablation_level=0, BASELINE): `mx_A_n500_s1` — R30: coop=94.0%, Gini=0.9653, B_RLHF=0.607, mean_wealth=183.25. event-aggregate B_RLHF (R1–R30) = 0.516. condA plateau confirmed R25–R30 (93.8–94.0% ± 0.3 pp, 6 consecutive rounds), confirming asymptotic saturation.

CONFIRMED THROUGH R30 (condB, ablation_level=5, FULL): `mx_B_n500_s2` — R30: coop=96.0%, Gini=0.9695, B_RLHF=0.627, mean_wealth=183.08. event-aggregate B_RLHF (R1–R30) = 0.545. condB peaked at R27 (coop=96.6%, B_RLHF=0.633) and oscillates near attractor R27–R30 (95.6–96.6%, B_RLHF 0.621–0.633).

KEY TRAJECTORY (condA vs condB per round): R5 76.0%/80.8% (B_RLHF 0.427/0.475); R10 85.6%/88.8% (0.523/0.555); R15 90.6%/91.6% (0.573/0.583); R20 92.6%/94.2% (0.593/0.609); R25 93.8%/95.4% (0.605/0.621); R27 93.8%/96.6% (0.605/0.633); R29 93.6%/95.4% (0.603/0.621); **R30 94.0%/96.0% (0.607/0.627)**. condB (full grounding, ablation_level=5) consistently leads condA (no grounding, ablation_level=0) by 2.0–4.8 pp throughout — directionally opposite to H2's null finding at N=100 (single seed, exploratory). Event-aggregate B_RLHF ordering: condB=0.545 > condA=0.516 (difference of 0.029). ADDITIONALLY: Padded control (Condition P, N=50, T=30) — all 3 seeds complete; 3-seed mean event-aggregate B_RLHF = 0.255 (§8.6). Full 10-seed N=500 statistics await sweep completion.**
...
| Metric | N=100, LLM, 10 seeds (R30) | N=500, LLM, 2 seeds partial (no rep_pen, terminal) | N=500, LLM, 1 seed/arm (R15, rep_pen=1.3) | **N=500, LLM, 1 seed/arm (R30, rep_pen=1.3, T=30 COMPLETE, 2026-06-05)** | N=500, Rule-based, 10 seeds (R30) | Amplification (R30 terminal / N=100) |
|--------|---------------------|-------------------------------------------|--------------------------------------------|------------------------------------------------------|-----------------------------|---------------------------------|
| Cooperation rate | 0.525±0.051 / 0.553±0.081 (A/B) | 0.956 | 0.906 / 0.916 | **0.940 (condA, plateau R25–R30) / 0.960 (condB, attractor)** | 0.386 (event-aggregate) | **1.79× / 1.83×** |
| Gini coefficient | 0.715±0.032 / 0.718±0.032 (A/B) | 0.943–0.952 | 0.816 / 0.835 | **0.9653 (condA) / 0.9695 (condB)** | 0.325 | **1.35× / 1.35×** |
| Round-level B_RLHF | 0.256±0.034 / 0.265±0.053 (A/B) | 0.623 | 0.573 / 0.583 | **0.607 (condA, saturated) / 0.627 (condB); peak condB R27/R28=0.633** | 0.101 | **2.37× / 2.45×** |
| Event-aggregate B_RLHF (R1–Rfinal) | ≈ 0.256 / ≈ 0.265 (A/B, R30) | ≈ 0.623 (partial) | — | **0.516 (condA, R1–R30) / 0.545 (condB, R1–R30)** | — | **2.01× / 2.13×** |
| Mean wealth | 174.6–177.3 | ≈ 180 | — | **183.25 (condA) / 183.08 (condB)** | — | — |

*"Partial" denotes 2-seed exploratory data with CUDA wedge interruptions. R15 and R30 gap-fill values are from `round_metrics.jsonl` (clean record, no EventLogger bug) for 1 seed per arm. Both R30 runs have 15,000 events (500 agents × 30 rounds); T=30 fully complete (R30 completed via OOM retry at chunk_size=1). Event-aggregate B_RLHF for the R30 column is computed from all 15,000 events using the full three-way TV formula. Round-level B_RLHF is computed from the single round's 500-agent action counts. Amplification column uses N=100 LLM terminal (0.195) as denominator. Note: event-aggregate B_RLHF (R1–Rfinal) averages over all rounds including early low-cooperation rounds, hence values < round-level at late rounds. condB peak was R27/R28 (coop=96.6%, B_RLHF=0.633); R30 terminal is 96.0%/0.627.*
...
- **H2** (ΔB_RLHF < 0 within Mistral-7B) is **confirmed at N=500** in the sense that B_RLHF(A) ≈ B_RLHF(B) ≈ 0.623 — both arms are equally high, which is consistent with "no grounding reduction" (the same null as N=100, but at a vastly higher absolute level). The R29 gap-fill sweep (complete as of 2026-06-05) partially addresses this: condB exceeded condA at every measured round R1–R29, which is consistent with grounding providing marginally stronger norm signals at N=500 — directionally opposite to H2's null at N=100. Whether this single-seed asymmetry represents a genuine cascade-form shift or sampling noise requires multi-seed confirmation (condA s2 / condB s3, currently running at T=15).
...
The gap-fill sweep relaunched 2026-06-03 on two P100 GPUs (`tmux: n500_condA` and `n500_condB`). **Round horizon:** mx_A_n500_s1 and mx_B_n500_s2 ran T=30 (**both confirmed through R30, T=30 COMPLETE, 2026-06-05**; R30 completed via OOM retry at chunk_size=1 after initial OOM at chunk_size=2; condA finished 02:30:20, condB finished 03:25:05 CEST). All remaining seeds run T=15 (see pre-registered deviation above). Seed schedule: Condition A — s1 (T=30, R30 complete); Condition B — s2 (T=30, R30 complete).
...
- **Cross-condition ordering: condB > condA at all measured rounds R1–R30.** Gap (B − A): 4.8 pp (R5) → 2.6 pp (R9) → 1.0 pp (R15) → 1.6 pp (R20) → 1.8 pp (R29) → **2.0 pp (R30)**; peak gap 2.8 pp (R27/R28). Directional pattern — grounded condition consistently at higher cascade level — is opposite to H2 and requires multi-seed confirmation.

**Process status (updated 2026-06-05):** T=30 FULLY COMPLETE for both s1/s2 gap-fill seeds. mx_A_n500_s1 finished at 02:30:20 CEST (R30: coop=94.0%, B_RLHF=0.607, Gini=0.9653); mx_B_n500_s2 finished at 03:25:05 CEST (R30: coop=96.0%, B_RLHF=0.627, Gini=0.9695). R30 completed via OOM retry at chunk_size=1 after initial CUDA OOM at chunk_size=2. Both tmux sessions now running the T=15 multi-seed sweeps: `n500_condA` running mx_A_n500_s2 (at R8/15 as of 2026-06-05 21:38 CEST); `n500_condB` running mx_B_n500_s3 (at R7/15 as of 20:08, likely R8 now).
