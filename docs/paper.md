...
**Execution plan (updated 2026-06-05).** Completed GPU-bound runs: (i) ✅ memory ablation LLM-policy (§8.5, H8) — 24/24 cells done, H8 falsified; (ii) ✅ padded-prompt control N=50 (§8.6) — all 3 seeds done; (iii) ✅ N=500 gap-fill sweep confirmed through R29 (§8.1.5). Remaining GPU items: (a) cross-cultural LLM at 10 seeds (§8.3, H9) — launcher-ready; (b) padded-prompt control at N=100 (closes AE1, §7.9); (c) cross-model comparison for Qwen2.5-7B and GPT-4o-mini (§6.6, H7 cross-family). Two items have external blockers: §8.4 (human Prolific baseline — IRB approval) and the multi-country ESS pooled refit (Limitation 13 — awaits ESS R11 MD multi-country release). N=500 multi-seed cascade (T3-C) is highest-value remaining experiment but requires ~20–30h GPU (see REMAINING_FOR_PAPER.md).
...
**Status: COMPLETE — 24/24 CELLS DONE (2026-06-05, 09:13 CEST).** V2 re-run (`tmux: h8_memory_ablation`, both bugs patched) ran all 24 cells; script reported "Total: 24 | Skipped: 0 | Failed: 0". All cells: M0G ✓ (3/3), M0U ✓ (3/3), M1G ✓ (3/3), M1U ✓ (3/3), M2G ✓ (3/3), M2U ✓ (3/3), M3G ✓ (3/3), M3U ✓ (3/3). Experiment IDs: `ablation_M{0-3}_{grounded,ungrounded}_s{42,123,7}_v2`. Analysis: `python scripts/analyze_memory_ablation.py --suffix _v2` (run 2026-06-05).
