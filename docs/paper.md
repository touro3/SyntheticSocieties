...
*Confirmed at α=0.05 family-wise: none. Confirmed at α=0.05 per-test (no correction): H5 (post-hoc design), H9 (rule-based proxy). Directional only: H1, H4 (H4 newly added: ΔQ = +0.005 at N=100, p = 0.68 — directionally consistent but not significant, pilot Q-shift not reproduced). Falsified at N=100: H2. Pending: H3, H6, H8 at LLM scale; H7 unaffected because design is separate. **§8.1 N=500 LLM-scale extension status (snapshot 2026-05-28).** The Condition-A and Condition-B N=500 cells (`experiments/mx_A_n500_s{1,6}/`, `experiments/mx_B_n500_s{1,6}/`) progressed across multiple resume cycles (rounds 8/16/21/23 reached at various points) but did not run to completion on the first attempts. Root cause was diagnosed on 2026-05-28: launching multiple `run_config_simulation.py` processes against the HuggingFace + bitsandbytes stack within the same second wedges all of them in a CUDA generation hang (100% GPU utilisation, zero syscall activity, SIGTERM ignored — only SIGKILL recovers them). A separate latent bug — `EventLogger` silently truncating `events.jsonl` when a bare (non-`--resume`) launch was issued against an existing exp_id — accounts for the size-zero event logs observed on the s6 cells; the logger now raises `FileExistsError` on non-empty event files unless `force=True` is explicitly passed. The four cells were relaunched sequentially with a 120 s stagger between processes and are currently resuming from their checkpoints; H1/H2/H3 disambiguation at primary LLM scale remains open pending completion of the four resumed cells. The follow-up critical-path runs (§8.5 memory ablation, §8.3 cross-cultural LLM, AE1 padded-prompt control at N=100, cross-model Qwen2.5-7B and GPT-4o-mini) are queued via `scripts/_queue_pending_experiments.sh` to dispatch sequentially on GPU 0 once the N=500 sweep finishes. The only N=500 cells with completed status as of this draft are Condition D (`mx_D_n500_s1..10`, §8.2), which the deterministic ESS-rule policy runs without GPU.*
