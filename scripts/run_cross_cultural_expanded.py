...
- Secondary WVS benchmark in output
...
def _run_single(
    ...
    if dry_run:
        ...
        # Synthetic: trust level determines base cooperation
    ...
    # Filter AT sample to proxy cluster trust range
    cohort_df = ess_df[
    ...
    if len(cohort_df) == 0:
        # Fallback: use nearest rows
        cohort_df = ess_df.nsmallest(
            max(10, n_agents),
            by=[abs(ess_df["trust_people"] - (cluster.trust_lo + cluster.trust_hi) / 2)
                if False else "trust_people"]
        )
...
def run_cluster_multiseed(
    ...
    for seed in seeds:
        ...
        r = _run_single(cluster, n_agents, n_rounds, policy_type, seed, ess_df, dry_run)
...
def _save_expanded(
    ...
    cc = compute_cross_cultural_correlation_multiseed(primary)
    ...
    payload: dict = {
        ...
        "correlation": {
            "pearson_r": round(cc.pearson_r, 4),
            "pearson_p": round(cc.pearson_p, 4),
            "spearman_rho": round(cc.spearman_rho, 4),
            "spearman_p": round(cc.spearman_p, 4),
            "gradient_recovered": cc.gradient_recovered,
        },
        "cluster_results": {},
        "robustness": {},
    }
...
def _save_csv(
    ...
    lines = [
        ...
        "std_coop,n_seeds,mean_gini,n_agents,pearson_r,spearman_rho,gradient_recovered"
    ]
    for r in sorted(multi_results, key=lambda x: x.ess_mean_trust):
        lines.append(
            ...
            f"{round(cc.pearson_r,4)},{round(cc.spearman_rho,4)},{cc.gradient_recovered}"
        )
    path.write_text("\n".join(lines) + "\n")
...
def main() -> None:
    ...
    # Run multi-seed simulations for each agent size
    multi_results_by_size: dict[int, list[ClusterMultiSeedResult]] = {}

    for sz in agent_sizes:
        is_primary = sz == n_agents_primary
        print(f"\n── Agent size: {sz} ({'primary' if is_primary else 'robustness'}) ──")
        ...
        print(f"\n  Pearson r = {cc.pearson_r:+.3f}  Spearman ρ = {cc.spearman_rho:+.3f}  "
              f"Gradient: {'YES ✓' if cc.gradient_recovered else 'NO ✗'}")
        multi_results_by_size[sz] = size_results

    # Save results
    _save_expanded(_RESULTS_PATH, multi_results_by_size, n_rounds, n_seeds)
    print(f"\nResults saved to: {_RESULTS_PATH}")

    primary_results = multi_results_by_size[n_agents_primary]
    cc_primary = compute_cross_cultural_correlation_multiseed(primary_results)
    _save_csv(_TABLE_PATH, primary_results, cc_primary)
