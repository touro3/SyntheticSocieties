...
def run_simulation(name, profiles, policy, backend, args, out_path, is_ablated=False):
    ...
    world = World(
        WorldState(), InstitutionManager(), NetworkManager.small_world(agent_ids, k=dense_k, rewiring_prob=0.0)
    )
...
def main():
    ...
    backend.load()
    run_simulation(
        "Condition A (Ablated - Dense)",
        profiles,
        ParallelPolicyTopology(backend),
        backend,
        args,
        out_dir / "condition_a_dense.parquet",
        is_ablated=True,
    )
    run_simulation(
        "Condition B (Grounded - Dense)",
        profiles,
        ParallelPolicyTopology(backend),
        backend,
        args,
        out_dir / "condition_b_dense.parquet",
        is_ablated=False,
    )
