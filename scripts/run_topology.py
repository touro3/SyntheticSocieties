...
def run_simulation(name, profiles, policy, backend, args, out_path, is_ablated=False):
    ...
    dense_k = len(agent_ids) - 1 # FULLY CONNECTED NETWORK
    world = World(WorldState(), InstitutionManager(), NetworkManager.small_world(agent_ids, k=dense_k, rewiring_prob=0.0))
    ...
    for round_id in range(args.rounds):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
            ...
            for f in concurrent.futures.as_completed(futures): prompts.append(f.result())
        ...
        for agent, response in zip(agents, responses):
            ...
            act = match.group(1) if match else ("cooperate" if "cooperate" in action_str else "save" if "save" in action_str else "work")
        ...
        for agent in agents:
            ...
            if act == "work": agent.state.wealth += 8.0
            elif act == "save": agent.state.wealth += 4.0
            elif act == "cooperate": agent.state.wealth -= 3.0
            if benefit_pool > 0: agent.state.wealth += benefit_pool / len(agents)
            logger.log_event({"round_id": round_id + 1, "agent_id": agent.profile.agent_id, "action": {"action_type": act}, "state_after": {"wealth": float(agent.state.wealth)}})
...
def main():
    ...
    parser.add_argument("--out-dir", type=str, default="experiments/topology_test")
    args = parser.parse_args()
    out_dir = Path(args.out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    loader = EmpiricalProfileLoader(args.artifact_dir); profiles = loader.load_population(target_size=args.pop_size)
    backend = FastBatchedBackend(temperature=0.5); backend.load()
    run_simulation("Condition A (Ablated - Dense)", profiles, ParallelPolicyTopology(backend), backend, args, out_dir / "condition_a_dense.parquet", is_ablated=True)
    run_simulation("Condition B (Grounded - Dense)", profiles, ParallelPolicyTopology(backend), backend, args, out_dir / "condition_b_dense.parquet", is_ablated=False)
