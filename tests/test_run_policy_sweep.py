...
from environment.policy_intervention import sensitivity_index
...
def _make_agent(agent_id: str = "a0", wealth: float = 50.0, last_action=None) -> Agent:
    ...
    return Agent(profile=profile, state=state, memory=MemoryBuffer(max_items=10), policy=MockPolicy())
...
def test_sweep_points_gini_in_unit_interval():
    results = run_sweep(n_agents=6, n_rounds=4)
    for sp in results:
        assert 0.0 <= sp.post_gini <= 1.0, f"post_gini out of [0,1] for param={sp.parameter}: {sp.post_gini}"
...
def test_sensitivity_index_negative_for_redistribute():
    ...
    assert si <= 0, f"Expected negative sensitivity index for redistribute_top_pct, got {si}"
...
def test_direction_recovery_from_sweep_results():
    ...
    assert sweep_checks[0].recovered, f"Direction not recovered: {sweep_checks[0].check} (Δ={sweep_checks[0].delta})"
